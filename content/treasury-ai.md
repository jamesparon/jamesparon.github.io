**JEL Classification:** G12, E43, H63, E62, O33\
...
Before turning to the valuation framework, we document the Treasury market's response to major AI model releases. Table 1 reports the average change in yields around the 17 AI model release dates identified by (Andrews and Farboodi 2025), using a $\pm$`<!-- -->`{=html}3 trading day window.[^5]

  Component                $\pm$`<!-- -->`{=html}3 days (bp)   $\pm$`<!-- -->`{=html}15 days (bp)
  ---------------------- ----------------------------------- ------------------------------------
  Nominal yield (10yr)               $-$`<!-- -->`{=html}6.6              $-$`<!-- -->`{=html}9.0
  Breakeven inflation                $-$`<!-- -->`{=html}1.5              $-$`<!-- -->`{=html}2.4
  ACM term premium                   $-$`<!-- -->`{=html}3.0              $-$`<!-- -->`{=html}3.1
...
<figure id="fig:event_path" data-latex-placement="ht">

<figcaption>Average path of the 10-year nominal Treasury yield around 17 major AI model releases (Andrews-Farboodi sample). Yields are normalized to zero on the event day. The decline begins before the release (likely reflecting pre-announcement leaks or anticipation) and persists for roughly two weeks.</figcaption>
</figure>
...
The CBO publishes "rules of thumb" that quantify these sensitivities.[^9] If productivity growth is 0.1 percentage points slower per year than projected, then over the 2026--2035 period:
...
For every dollar of revenue lost to slower growth, spending falls by just eight cents. We compute the implied elasticities as the ratio of the percentage change in cumulative nominal revenue (or spending) to the percentage change in cumulative nominal GDP over the CBO's 10-year window. Under the $-$`<!-- -->`{=html}0.1pp productivity scenario, cumulative nominal GDP over 2026--2035 falls by 0.79% (from \$373.2 trillion to \$370.2 trillion), while cumulative revenue falls by 0.85% (\$572 billion on a \$67.5 trillion base). The revenue elasticity is therefore $0.85/0.79 = 1.07$.
...
<figure id="fig:rev_spend" data-latex-placement="ht">

<figcaption>Projected federal revenue and primary spending as a share of GDP, 2026–2055. Under the CBO baseline, spending exceeds revenue throughout (persistent primary deficits). Under the +0.5pp AI scenario, the gap between spending and revenue narrows sharply but does not close by 2055, as Medicare costs rise with GDP in the long run. Revenue rises because of real bracket creep (<span class="math inline"><em>ε</em> = 1.07</span>); the spending share falls because most non-health categories are inelastic to growth.</figcaption>
</figure>

# AI Growth Scenarios and Treasury Valuations

We take the CBO's baseline projections and ask: what happens to the fundamental value of government debt if AI raises productivity growth by 0.1 or 0.5 percentage points per year?[^13] The terminal value is calibrated to match the market price of debt in the baseline, as described in equation [\[eq:tv_star\]](#eq:tv_star).

We use the CBO's 30-year budget projections from the Long-Term Budget Outlook, apply the CBO-implied elasticities to compute alternative surplus paths, and roll forward the debt using the CBO's baseline interest rate assumptions. In each AI scenario, we also allow the real interest rate to rise with productivity growth, using the pass-through implied by the CBO's own rules-of-thumb workbook.[^14] For the terminal value we impose *complete* long-run pass-through, $\Delta r = \Delta g$, so the Gordon growth denominator $(r-g)$ is held fixed across scenarios; the terminal value then scales only with the level of GDP and the year-30 discount factor, as used in equation [\[eq:tv_star\]](#eq:tv_star) and discussed in Section 4. This ensures that our discount rates are consistent with the CBO's own view of how growth affects the equilibrium real rate.

  -------------- -------------------------- -------------- ------------- ------------- --------------
  Scenario                           PDV(S)   $TV^*$ disc.   Fundamental   $\Delta$Val    $\Delta$Val
                                      (\$T)          (\$T)   Value (\$T)         (\$T)   (% of $D_0$)
  CBO Baseline     $-$`<!-- -->`{=html}12.5           44.6          32.1           ---            ---
  AI +0.1pp        $-$`<!-- -->`{=html}11.3           44.6          33.3         +1.19           +3.7
  AI +0.5pp         $-$`<!-- -->`{=html}6.7           44.5          37.8         +5.72          +17.8
  -------------- -------------------------- -------------- ------------- ------------- --------------

  : Fundamental value of U.S. government debt under AI growth scenarios. Following (Jiang et al. 2022), the terminal value $TV^*$ is calibrated so that the CBO baseline reproduces the market price of debt ($D_0 = \$32.1$T). Under AI scenarios, the market-implied post-2055 surplus-to-GDP ratio (12.4%, far above the CBO's projected $-$`<!-- -->`{=html}2.2%) is held fixed, so $TV^*$ scales with the terminal GDP level. Within-horizon discount rates incorporate the CBO's implied pass-through from productivity growth to real interest rates. The terminal value is recapitalized using the scenario-specific year-30 discount factor and long-run rate, and all discount rates include a 2.6 percentage point GDP risk premium (following (Jiang et al. 2022)).
...
Table 4 shows how much larger the gains would be without the real interest rate pass-through. Under fixed discount rates, the +0.1pp scenario delivers \$2.4 trillion instead of \$1.19 trillion, and the +0.5pp scenario delivers \$12.5 trillion instead of \$5.7 trillion. The difference is driven entirely by the terminal value: when rates stay low, the GDP-scaled post-horizon surpluses are capitalized at unchanged rates, so $TV^*$ rises with the GDP level rather than falling.
  ...
  Scenario                           PDV(S)   $TV^*$ disc.   Fundamental   $\Delta$Val    $\Delta$Val
                                      (\$T)          (\$T)   Value (\$T)         (\$T)   (% of $D_0$)
  CBO Baseline     $-$`<!-- -->`{=html}12.5           44.6          32.1           ---            ---
  AI +0.1pp        $-$`<!-- -->`{=html}11.4           45.9          34.5          +2.4           +7.4
  AI +0.5pp         $-$`<!-- -->`{=html}7.0           51.6          44.6         +12.5          +38.9
  -------------- -------------------------- -------------- ------------- ------------- --------------
...
<figure id="fig:debtgdp_main" data-latex-placement="ht">

<figcaption>Projected debt-to-GDP ratio under CBO baseline and AI growth scenarios, 2025–2055, with CBO-implied increase in real interest rates. Under the +0.5pp scenario, debt-to-GDP slows to 141% by 2055.</figcaption>
</figure>

Our debt-to-GDP projections are consistent with the CBO's own sensitivity analysis. In its May 2025 report on alternative budget scenarios, the CBO projects that 0.5 percentage points of higher productivity growth would reduce the 2055 debt-to-GDP ratio from 156% to 113% --- a 43 percentage point improvement.[^15]
...
To translate these value changes into yield terms, we use the weighted average duration of the outstanding Treasury portfolio (70.4 months, or 5.87 years, from the TBAC presentation of December 2025). Table 5 reports the approximate change in nominal yields implied by each panel.
  ...
  ----------- ------------------------- ------------------------- --
  AI +0.1pp                        +3.7    $-$`<!-- -->`{=html}63 
  AI +0.5pp                       +17.8   $-$`<!-- -->`{=html}304 
...
<figure id="fig:debtgdp_lower" data-latex-placement="ht">

<figcaption>Projected debt-to-GDP ratio incorporating the immediate decline in nominal Treasury yields from improved fiscal fundamentals. The full yield decline (63bp for +0.1pp, 304bp for +0.5pp) is applied from 2026 onward. Lower yields reduce interest expense, creating a virtuous cycle that amplifies the fiscal benefit of growth. Compare with Figure <a href="#fig:debtgdp_main" data-reference-type="ref" data-reference="fig:debtgdp_main">3</a>, which holds yields at the CBO baseline.</figcaption>
</figure>
...
We approximate their calibration with a smooth GDP path that reaches +0.5% by 2030, +4% by 2040, and +15% by 2055 relative to the CBO baseline.[^16] We calibrate the within-horizon CBO interest rate pass-through to the time-varying extra growth implied by that level path. In line with the permanent-growth calibration, the terminal value recapitalization includes the scenario-implied nominal growth rate in the Gordon denominator.

  ------------------ -------------------------- -------------- ------------- ------------- --------------
  Scenario                               PDV(S)   $TV^*$ disc.   Fundamental   $\Delta$Val    $\Delta$Val
                                          (\$T)          (\$T)   Value (\$T)         (\$T)   (% of $D_0$)
  CBO Baseline         $-$`<!-- -->`{=html}12.5           44.6          32.1           ---            ---
  Permanent +0.1pp     $-$`<!-- -->`{=html}11.3           44.6          33.3         +1.19           +3.7
  Permanent +0.5pp      $-$`<!-- -->`{=html}6.7           44.5          37.8         +5.72          +17.8
  Jones-Tonetti         $-$`<!-- -->`{=html}8.3           42.2          33.8         +1.71           +5.3
  ------------------ -------------------------- -------------- ------------- ------------- --------------
...
|                                  | AI +0.1pp                                   | AI +0.5pp                                   |
+:=================================+==================:+========================:+==================:+========================:+
| 2-3 (lr)4-5 Assumption           | $\Delta$Val (\$T) | $\Delta y$ (bp)         | $\Delta$Val (\$T) | $\Delta y$ (bp)         |
+----------------------------------+-------------------+-------------------------+-------------------+-------------------------+
| Temporary ($r$, $g$ revert)      | +2.39             | $-$`<!-- -->`{=html}127 | +12.49            | $-$`<!-- -->`{=html}663 |
+----------------------------------+-------------------+-------------------------+-------------------+-------------------------+
| Permanent, $\Delta r = \Delta g$ | +1.19             | $-$`<!-- -->`{=html}63  | +5.72             | $-$`<!-- -->`{=html}304 |
+----------------------------------+-------------------+-------------------------+-------------------+-------------------------+
| Permanent, CBO literal           | +1.12             | $-$`<!-- -->`{=html}59  | +5.38             | $-$`<!-- -->`{=html}286 |
+----------------------------------+-------------------+-------------------------+-------------------+-------------------------+
...
<figure id="fig:spending_share_ww2" data-latex-placement="ht">
<img src="../figures/spending_share_since_ww2.png" style="width:95.0%" />
<figcaption>Composition of U.S. federal outlays, FY1945–FY2056. Shares of total spending going to discretionary appropriations (blue), mandatory programs (red), and net interest (yellow). Historical data: OMB Table 3.1 reconstruction through FY1961 and OMB Table 8.3 BEA categories FY1962–FY2024. Projections: CBO February 2026 Long-Term Budget Outlook (publication 62044) for FY2025–FY2056.</figcaption>
</figure>
...
<figure id="fig:debtgdp" data-latex-placement="ht">

<figcaption>Projected debt-to-GDP ratio under CBO baseline and AI growth scenarios. The CBO baseline assumes 1.8% real GDP growth. AI scenarios add 0.1 and 0.5 percentage points to productivity growth, with revenue and spending responses computed using CBO-implied elasticities. Debt dynamics incorporate the CBO-implied increase in real interest rates with productivity growth.</figcaption>
</figure>
...
<figure id="fig:pvtg_fixed" data-latex-placement="ht">

<figcaption>Change in the present value of federal revenue and primary spending as a function of extra productivity growth, holding discount rates fixed at the baseline level. Revenue rises steeply (<span class="math inline"><em>ε</em> = 1.07</span>), while spending rises only modestly (Social Security and, in the long run, Medicare scale with growth, but most other categories are inelastic). The vertical gap between the two lines is the improvement in the present value of surpluses.</figcaption>
</figure>
...
<figure id="fig:pvtg" data-latex-placement="ht">

<figcaption>Change in the present value of federal revenue and primary spending as a function of extra productivity growth, incorporating the CBO-implied increase in real rates with productivity growth. Revenue rises modestly because higher discount rates partially offset the elasticity effect. Spending <em>falls</em> because inelastic spending is discounted at higher rates. Compare with Figure <a href="#fig:pvtg_fixed" data-reference-type="ref" data-reference="fig:pvtg_fixed">8</a>.</figcaption>
</figure>
...
- **10-year baseline projections (February 2026).** Revenue, outlays (by category), net interest, and debt, 2026--2036.\
  Source: [US-CBO/eval-projections](https://github.com/US-CBO/eval-projections) on GitHub.\
  File: , filtered to .\
  Publication: [The Budget and Economic Outlook: 2026 to 2036](https://www.cbo.gov/publication/61882) (CBO Publication 61882).
...
- **30-year extended baseline projections (February 2026).** Revenue, Social Security, Medicare, Medicaid, other mandatory, discretionary, net interest, debt/GDP, and GDP, 2026--2056 (all as % of GDP).\
  Source: [The Long-Term Budget Outlook Data: 2026 to 2056](https://www.cbo.gov/publication/62044) (CBO Publication 62044).\
...
- **Historical GDP and economic indicators.** Nominal GDP, real GDP, inflation, interest rates, 1949--2035.\
  Source: [CBO Budget and Economic Data](https://www.cbo.gov/data/budget-economic-data) page.\
  File: , extracted to .

## CBO Rules of Thumb 

- **Budget sensitivity to economic conditions.** Elasticities of revenue and spending to productivity growth, labor force growth, interest rates, and inflation.\
  Source: [How Changes in Economic Conditions Might Affect the Federal Budget: 2025 to 2035](https://www.cbo.gov/publication/61198) (CBO Publication 61198, March 2025).

- **Interactive workbook.** User-adjustable scenarios showing the effect of productivity changes on interest rates, revenue, and outlays. Used to extract the CBO-implied pass-through from productivity growth to real interest rates.\
  Source: [Workbook for How Changes in Economic Conditions Might Affect the Federal Budget: 2025 to 2035](https://www.cbo.gov/publication/61183) (CBO Publication 61183, March 2025).\
  File: , sheet "5. Productivity_CBO."
...
- **Long-run Medicare cost growth.** Used to calibrate the long-run income elasticity of Medicare spending to productivity. The Trustees project per-beneficiary Medicare cost to grow with GDP per capita plus a slowly declining excess-cost wedge, which implies a unit elasticity (the Medicare-to-GDP share is invariant to the productivity perturbation in the long run); we phase this in over the 30-year horizon, holding it near zero within the CBO's 10-year window (Section 4.1).\
  Source: [2026 Annual Report of the Boards of Trustees of the Federal Hospital Insurance and Federal Supplementary Medical Insurance Trust Funds](https://www.cms.gov/oact/tr/2026) (Centers for Medicare & Medicaid Services, released June 9, 2026).\
...
- **Zero-coupon spot rates (2025Q1).** Used to construct discount factors at maturities 1--30 years.\
  Source: [US-CBO/discount-factors](https://github.com/US-CBO/discount-factors) on GitHub.\
  File: , filtered to .\
...
- **Weighted average maturity: 70.4 months (December 2025).** Used to translate value changes into implied yield changes.\
  ...
  Available at [Treasury TBAC page](https://home.treasury.gov/policy-issues/financing-the-government/quarterly-refunding/treasury-borrowing-advisory-committee-tbac).

## CBO Long-Term Sensitivity Analysis 

- **Debt/GDP under alternative productivity assumptions.** CBO projects that $\pm$`<!-- -->`{=html}0.5pp of productivity growth changes the 2055 debt/GDP ratio by $\mp$`<!-- -->`{=html}43 percentage points relative to the baseline of 156%.\
  Source: [The Long-Term Budget Outlook Under Alternative Scenarios for the Economy and the Budget](https://www.cbo.gov/publication/61429) (CBO Publication 61332, May 2025).
...
Andrews, Isaiah, and Maryam Farboodi. 2025. "Pricing Transformative AI." Unpublished manuscript.
...
Chow, Trevor, Basil Halperin, and J. Zachary Mazlish. 2025. "Transformative AI, Existential Risk, and Real Interest Rates." Unpublished manuscript.

Cochrane, John H. 2023. *The Fiscal Theory of the Price Level*. Princeton University Press.

Du, Wenxin, Joanne Im, and Jesse Schreger. 2018. "The U.S. Treasury Premium." *Journal of International Economics* 112: 167--81.

Eisfeldt, Andrea L., Gregor Schubert, and Miao Ben Zhang. 2023. "Generative AI and Firm Values." Unpublished manuscript.

Gómez-Cram, Roberto, Howard Kung, and Hanno Lustig. 2025. "Can U.S. Treasury Markets Add and Subtract?" Unpublished manuscript.

Gómez-Cram, Roberto, Howard Kung, Hanno Lustig, and David Zeke. 2025. "Fiscal Redistribution Risk in Treasury Markets." Unpublished manuscript.

Greenwood, Robin, Samuel G. Hanson, and Jeremy C. Stein. 2015. "A Comparative-Advantage Approach to Government Debt Maturity." *Journal of Finance* 70 (4): 1683--722.
...
Hall, George J., and Thomas J. Sargent. 2011. "Interest Rate Risk and Other Determinants of Post-WWII U.S. Government Debt/GDP Dynamics." *American Economic Journal: Macroeconomics* 3 (3): 192--214.
...
Jiang, Zhengyang, Arvind Krishnamurthy, and Hanno Lustig. 2021. "Foreign Safe Asset Demand and the Dollar Exchange Rate." *Journal of Finance* 76 (3): 1049--89.

Jiang, Zhengyang, Hanno Lustig, Stijn Van Nieuwerburgh, and Mindy Z. Xiaolan. 2022. "Measuring U.S. Fiscal Capacity Using Discounted Cash Flow Analysis." *Brookings Papers on Economic Activity* 53 (2 (Fall)): 157--229.

Jiang, Zhengyang, Hanno Lustig, Stijn Van Nieuwerburgh, and Mindy Z. Xiaolan. 2024a. "The U.S. Public Debt Valuation Puzzle." *Econometrica* 92 (4): 1177--215.

Jiang, Zhengyang, Hanno Lustig, Stijn Van Nieuwerburgh, and Mindy Z. Xiaolan. 2024b. "What Drives Variation in the u.s. Debt-to-Output Ratio? The Dogs That Didn't Bark." *Journal of Finance* 79 (4): 2603--65.

Jones, Charles I. 2024. "The A.I. Dilemma: Growth Versus Existential Risk." *American Economic Review: Insights* 6 (4): 575--90.
...
Mian, Atif, Ludwig Straub, and Amir Sufi. 2021. "Indebted Demand." *Quarterly Journal of Economics* 136 (4): 2243--307.

Miller, Max, James D. Paron, and Jessica A. Wachter. 2025. "Sovereign Default and the Decline in Interest Rates." Unpublished manuscript.
...
Rachel, Łukasz. 2025. "What Next for $r^*$? A Capital Market Equilibrium Perspective on the Natural Rate of Interest." *Brookings Papers on Economic Activity*.

Sims, Christopher A. 1994. "A Simple Model for Study of the Determination of the Price Level and the Interaction of Monetary and Fiscal Policy." *Economic Theory* 4 (3): 381--99.

Woodford, Michael. 2001. "Fiscal Requirements for Price Stability." *Journal of Money, Credit and Banking* 33 (3): 669--728.

[^1]: London Business School. Email: hkung@london.edu.

[^2]: Stanford Graduate School of Business and NBER. Email: hlustig@stanford.edu.

[^3]: Stanford Graduate School of Business. Email: jparon@stanford.edu

[^4]: We thank Christopher Tonetti for helpful comments on an early draft of the paper.

[^5]: Data from FRED: DGS10 (nominal yield), DFII10 (TIPS yield), T10YIE (breakeven inflation), DAAA (Moody's AAA yield). The ACM term premium is from the New York Fed (FRED series THREEFYTP10). The expected rate is the nominal yield minus the term premium.

[^6]: This framework is developed in (Jiang et al. 2022). See also (Jiang et al. 2024b).

[^7]: The GDP risk premium compensates investors for bearing the risk that surpluses will be low precisely when the economy is weak and the marginal utility of wealth is high. (Jiang et al. 2024a) derive the appropriate discount rate from a no-arbitrage framework and show that the market value of U.S. debt exceeds its fundamental value. (Jiang et al. 2022) estimate the risk premium from the covariance of surplus growth with aggregate consumption growth. We use their estimates, which imply a risk premium of approximately 2.6 percentage points over Treasurys.

[^8]: We use the CBO's 30-year baseline projections from the Long-Term Budget Outlook (Publication 62044, February 2026), which extends the 10-year baseline through 2056. Discount factors are constructed from the Treasury yield curve published in the `US-CBO/discount-factors` repository, augmented with the GDP risk premium.

[^9]: Congressional Budget Office, "How Changes in Economic Conditions Might Affect the Federal Budget: 2025 to 2035," Publication 61198, March 2025.

[^10]: Total SS spending over 2026--2035 is \$21.4 trillion. If all \$43 billion of the mandatory change is attributed to SS: $\varepsilon_{SS} = (43/21{,}420) / (2{,}946/373{,}164) = 0.25$.

[^11]: The calibrated $D$ implies an average benefit duration of roughly 18 years, consistent with SSA data on average claiming age ($\approx$`<!-- -->`{=html}64) and life expectancy for beneficiaries ($\approx$`<!-- -->`{=html}84). By year 20, the elasticity reaches 0.66; by year 30, it reaches 0.81.

[^12]: 2026 Annual Report of the Boards of Trustees of the Federal Hospital Insurance and Federal Supplementary Medical Insurance Trust Funds: long-run per-beneficiary cost is projected to grow with GDP per capita plus a declining excess-cost wedge, implying a unit income elasticity with the Medicare-to-GDP share roughly constant. Within CBO's 10-year rule-of-thumb window our model matches CBO's treatment (the mandatory-health response is $\approx 0$; the \$43 billion mandatory figure above is essentially all Social Security), and the unit elasticity operates only beyond that window. Holding Medicare at a zero elasticity instead --- letting its share *fall* below the Trustees' baseline as the economy grows --- would raise the headline +0.1pp gain from \$1.19 trillion to \$1.35 trillion.

[^13]: For context, the CBO's baseline assumes long-run real GDP growth of 1.8% per year. An additional 0.5 percentage points would bring growth to 2.3%, modestly above the post-2000 average.

[^14]: Congressional Budget Office, "Workbook for How Changes in Economic Conditions Might Affect the Federal Budget: 2025 to 2035," Publication 61183, March 2025. When productivity growth is permanently 0.1 percentage points lower, the 10-year Treasury rate falls by 1.2 basis points in year 1, ramping to 10.9 basis points by year 10 --- close to full pass-through ($\Delta r \approx \Delta g$) in the long run. We apply this partial, gradually ramping pass-through within the 30-year projection horizon.

[^15]: Congressional Budget Office, "The Long-Term Budget Outlook Under Alternative Scenarios for the Economy and the Budget," Publication 61332, May 2025.

[^16]: We interpolate the Jones-Tonetti anchor points (4% by 2040, 19% by 2060) using a monotone cubic spline, and linearly interpolate to 2055.

[^17]: Total-return indices (including distributions) from LSEG Datastream; the Datastream return index matches CRSP total returns to within rounding over the period CRSP also covers.
