...
First, I present new, well-identified evidence of declining innovation productivity within firms. To be sure, this is not the first paper to argue that new ideas are getting harder to find. points out that stable economic growth, coupled with an exponential increase in the number of researchers, must mean innovation productivity per researcher is falling. In support of this intuition, documents that, over time, innovation has required larger teams of more educated, older researchers. presents evidence that ideas have been getting harder to find in the U.S. for decades, and that this explains the recent growth slowdown. compile micro-level evidence of declining innovation productivity, demonstrating that the macro-level argument of is also true within product markets and firms.
...
My findings connect the secular decline in growth with the literature on the secular rise in the value of the stock market (**farhi2018accounting?**; **miller2025sovereign?**; **greenwald2025wealth?**; **atkeson2024end?**; **eggertson2021kaldor?**; **cho2024present?**). I make four contributions. First (and foremost), I present new micro-level evidence and a new mechanism to explain this rise. Second, while past studies have taken growth to be an exogenous input, I endogenize both growth and valuations to study their general-equilibrium relationship. Third, I depart from the representative-firm assumption in these papers and instead model the full distribution of individual firms. Because reallocation between firms is key to explaining the rise in valuations, a representative-firm model cannot possibly explain these dimensions of the data. And fourth, because I model individual firms, I distinguish the stock market from the consumption claim. This distinction turns out to have major welfare implications: the reallocation-driven growth in the stock market came at the expense of wages and future firms, implying that the present value of consumption (welfare) stagnated with output.[^15]

A separate asset-pricing literature studies the implications of firm heterogeneity and innovation for the equity risk premium (**garleanu2012displacement?**; **corhay2020competition?**; **loualiche2024asset?**) and aggregate return volatility (**cochrane2008two?**; **martin2013lucas?**). While this literature has so far focused on high-frequency volatility and risk premia, I show that firm heterogeneity is important even for determining the very-long-run *level* of market valuations. Indeed, all of the findings of this paper remain true even in an economy without any systematic risks or non-standard preferences.

Finally, this work builds on and contributes to the literature on firm dynamics and endogenous growth, the foundations of which are laid in , , and . My framework builds primarily on the Schumpeterian growth model of and . This class of endogenous-growth model has been extended to study numerous aspects of the economy, including markups and misallocation (**peters2020heterogeneous?**) and optimal innovation policy (**acemoglu2018innovation?**). My model studies a new tradeoff within firms---R&D versus M&A---in an economy with ex ante heterogeneous firm-level productivity (**hopenhayn1992entry?**; **luttmer2011mechanics?**). I use the model to study the economic transition resulting from a new structural change: new ideas getting harder to find.

Several firm-dynamics papers have also studied long-run structural change in the economy, with a particular interest in falling growth and rising concentration. show how declining population growth reduces firm entry and increases concentration.[^16] and propose that technological changes enabled some firms to scale up more easily, leading to rising concentration.[^17]$^{,}$[^18] and argue that a decline in knowledge diffusion between leaders and laggards explains these trends.[^19] This literature has by and large not considered asset prices. A notable exception is , who argue that a decline in discount rates (i.e., a rise in firm-level valuations) allows leaders to escape competition by laggards, until eventually both firms stop innovating.
...
In Appendix 10.1.1, I show that a decline of equal magnitude can be seen in the patent value measure of . Thus, the decline shows up in innovation outputs as well. I discuss how their measure relates to R&D, and why R&D is my preferred measure.
...
Reallocation could increase the value of the stock market if it means that high-valuation firms are growing larger. Consider the aggregate value-to-sales ratio $V_t/Y_t$. This aggregate ratio can be written as a sales-weighted average of firm-level value-to-sales ratios $V_{it}/p_{it}Y_{it}$: $$\begin{equation}
\frac{V_t}{Y_t} = \sum_{i=1}^{M_t}\frac{p_{it}Y_{it}}{Y_t}\times\frac{V_{it}}{p_{it}Y_{it}}.
\label{eq:xrdsale_weightedsum}
\end{equation}$$ The aggregate ratio could rise either because firm-level value-to-sales ratios rise, or because the sales shares of high-valuation firms increase (reallocation).

I decompose these two channels using the following accounting identity. Let $\mathcal{I}$ denote the set of firms (ignoring entry and exit for now). The change in ([\[eq:xrdsale_weightedsum\]](#eq:xrdsale_weightedsum)) from $t-1$ to $t$ can be written[^22] $$\begin{equation}
...
\label{eq:valsale_decomp}
\end{equation}$$ The within-firm change measures the effect of changes in firm-level valuations, holding fixed the sales shares of firms; the compositional change measures the effect of changing sales shares, holding fixed valuations. The operator $(\overline{\hphantom{}\cdot\vphantom{X}\hphantom{}})$ denotes an index averaging over the two periods: for example, $$\begin{equation}
...
Panel A of Figure 3 plots the cumulative change in aggregate value-to-sales since 1970 alongside the cumulative changes attributable to each of the channels in ([\[eq:valsale_decomp\]](#eq:valsale_decomp)). While most high-frequency changes in aggregate value-to-sales come from within-firm changes, the cumulative within-firm change has been small. Most of the long-run increase in the aggregate is instead explained by compositional change. In other words, there has been a sizable reallocation of revenues toward high-valuation firms. Reallocation explains the booming market.

Notably, the large and positive compositional change in the late 1990s coincided with the M&A wave (Figure 2). This is consistent with existing evidence that acquiring firms have significantly higher valuation ratios than target firms and that the merged value of the target and acquirer is significantly higher than their values before the merger (**betton2008corporate?**). See Appendix 10.1.3 for a detailed survey of this evidence.

The rise in aggregate value-to-sales is not simply about higher profits; aggregate value has also risen relative to *cashflows*. Panel B of Figure 3 plots the analogous decomposition for aggregate cashflow-to-value $D_t/V_t$: $$\begin{equation}
...
\label{eq:cfval_decomp}
\end{equation}$$ where $\mathcal{I}_{t-1,t}$ is the set of firms operating at $t-1$ or $t$. I decompose cashflow-to-value, instead of value-to-cashflow, simply because firm-level value-to-cashflow is undefined in periods with zero cashflows. The finding is the same: within firms, valuations have been relatively stable, while high-valuation (low cashflow-to-value) firms have gained substantial market share.
...
Individuals have identical log utility over consumption $c_t$ and constant-Frisch disutility over labor hours $l_t$. The lifetime utility of a single individual takes the form $$\begin{equation}
U_t \equiv \mathbb{E}_t\left[\int_0^\infty e^{-\rho\tau}\left(\log{c_{t+\tau}}-\chi\frac{l_{t+\tau}^{1+1/\zeta}}{1+1/\zeta}\right)d\tau\right],
\label{eq:lifetimeutility}
\end{equation}$$ where $\rho$ is the rate of time preference (impatience). Log utility corresponds to an elasticity of intertemporal substitution (EIS) of one. The parameter $\chi$ governs the overall disutility of working; the Frisch elasticity $\zeta$ governs the sensitivity of labor supply to changes in wages.

#### Demand for goods (market structure)

An individual's total consumption $c_t$ is a bundle of differentiated goods. In particular, firms produce a mass $N_t$ of varieties, indexed $j$. The individual's final demand for quantities $c_{jt}$ of these goods are then aggregated as $$\begin{equation}
c_t \equiv \left(\int_0^{N_t}c_{jt}^{1-1/\eta} dj\right)^{1/(1-1/\eta)},
\label{eq:cesdemand}
\end{equation}$$ subject to the budget constraint and taking each good's price as given. The parameter $\eta$ is the elasticity of substitution between varieties: the higher is $\eta$, the more substitutable.

Within variety $j$, $m_{jt}\in\mathbb{N}$ firms, indexed $i$, possess a blueprint to produce the good. The household views these blueprints as perfect substitutes, but with potentially heterogeneous qualities, denoted $q_{ijt}$. Hence, the total consumption of good $j$ is the sum $$\begin{equation}
c_{jt} \equiv \sum_{i=1}^{m_{jt}}q_{ijt}c_{ijt},
\label{eq:demand_withinvariety}
\end{equation}$$ where $c_{ijt}$ is the quantity of firm $i$'s output consumed by the household. Demand ([\[eq:cesdemand\]](#eq:cesdemand)) and ([\[eq:demand_withinvariety\]](#eq:demand_withinvariety)) will ultimately determine the competitive structure of product markets.

#### Labor supply

A fraction $\bar{s}$ of individuals are endowed with skilled labor. They can supply labor hours either to an incumbent firm for a wage $w_{{\rm x}t}$ or to a private effort to start a new firm (entrepreneurship). Entrepreneurship is detailed below. The household decides the mix of hours between these tasks. The remaining fraction $1-\bar{s}$ can supply production labor for a wage $w_{{\rm p}t}$. While the fraction of laborers $\bar{s}$ dedicated to skilled labor tasks is fixed, it is straightforward to endogenize this separation using a Roy model, as in .
...
Household consumption allocations $\{c_{ijt}\}$ maximize utility ([\[eq:cesdemand\]](#eq:cesdemand)) and ([\[eq:demand_withinvariety\]](#eq:demand_withinvariety)). Aggregating to total consumption (and production) $Y_{ijt}$, we have demand curves $$\begin{equation}
p_{ijt} = q_{ijt}\left(\frac{Y_{jt}}{Y_t}\right)^{-1/\eta}p_t.
\label{eq:hhdemandcurve_final}
\end{equation}$$ Aggregate output is the numeraire, so the aggregate price index $p_t=1$. Note that this demand restriction means that any two firms $i$ and $i'$ producing a version of variety $j$ must equate *quality-adjusted* prices: $$\begin{equation}
\frac{p_{ijt}}{q_{ijt}} = \frac{p_{i'jt}}{q_{i'jt}},
\label{eq:hhdemandcurve_withinvariety}
\end{equation}$$ consistent with the assumption of perfect substitution within product markets.

Firms choose prices subject to ([\[eq:hhdemandcurve_final\]](#eq:hhdemandcurve_final)) and ([\[eq:hhdemandcurve_withinvariety\]](#eq:hhdemandcurve_withinvariety)) to maximize operating profits ([\[eq:firmprofits_1\]](#eq:firmprofits_1)). Under Bertrand competition, only the firm with the lowest quality-adjusted marginal cost $w_{{\rm p}t}/(a_iq_{ijt})$ will produce variety $j$. We call this producer the *leader* and index it by $i_1$; we call the firm with the next lowest quality-adjusted marginal cost the *follower* with index $i_2$. The leader sets its price either to maximize its monopoly profits or, if necessary, to dissuade the follower from producing. Specifically, the leader's markup over marginal cost equals $$\begin{equation}
\mu_{ijt} \equiv \frac{p_{ijt}}{w_{{\rm p}t}/a_i} = \min\left\{\frac{a_{i_1}q_{i_1jt}}{a_{i_2}q_{i_2jt}},\frac{\eta}{\eta-1}\right\}.
\end{equation}$$ The term $\eta/(\eta-1)$ is the monopoly markup: the optimal markup in the presence of no competitor. The term $(a_{i_1}q_{i_1jt})/(a_{i_2}q_{i_2jt})$ is the marginal-cost advantage of the leader: if the marginal-cost advantage is sufficiently small, then the leader must engage in limit pricing by setting its quality-adjusted price equal to the quality-adjusted marginal cost of the follower.
...
The above conditions together imply that the revenue share of firm $i$ producing variety $j$ equals $$\begin{equation}
...
\label{eq:salesshare}
\end{equation}$$ The aggregates $\{\bar{a},Q_t,\mathcal{M}_{\eta-1}\}$ are averages of $\{a_i,q_{ijt},\mu_{ijt}\}$, defined below in ([\[eq:aggquality\]](#eq:aggquality)), ([\[eq:aggproductivity\]](#eq:aggproductivity)), and ([\[eq_app:Mcal_2\]](#eq_app:Mcal_2)). Because $\eta>1$, a firm generates more revenue for a product than average if it has higher productivity and quality. It earns lower revenues if it charges a higher markup, because high prices reduce demand ([\[eq:hhdemandcurve_final\]](#eq:hhdemandcurve_final)). Operating profits can be expressed in terms of revenues and markups as $$\begin{equation}
\Pi_{ijt} = \left(1-\frac{1}{\mu_{ijt}}\right)p_{ijt}Y_{ijt}.
\label{eq:operatingprofits}
\end{equation}$$

#### Aggregate output and wages

Aggregate output (and consumption) equals $$\begin{equation}
Y_t = A_tL_{{\rm p}t},
\end{equation}$$ where total-factor productivity (TFP, also output per labor hour) is defined as $$\begin{equation}
A_t  \equiv \underbrace{\phantom{x}N_t^{1/(\eta-1)}\phantom{x}}_{\textrm{variety}} \hphantom\times\hphantom \underbrace{\phantom{x}Q_t\phantom{x}}_{\textrm{quality}} \hphantom\times \underbrace{\phantom{x_t}\bar{a}\phantom{x_t}}_{\textrm{productivity}} \times \underbrace{\phantom{x_t}\Omega\phantom{x_t}}_{\textrm{static misalloc.}}.
\label{eq:aggtfp}
\end{equation}$$ The first component of TFP is the love-of-variety effect, which is magnified if varieties are less substitutable ($\eta$ is closer to one). The second term is *aggregate quality*, which is defined as an average of blueprint qualities:[^34] $$\begin{equation}
Q_t \equiv \left(\frac{1}{N_t}\int_0^{N_t}q_{jt}^{\eta-1}dj\right)^{1/(\eta-1)}.
\label{eq:aggquality}
\end{equation}$$ The latter two components capture the allocation of resources across producers. *Aggregate productivity* is[^35] $$\begin{equation}
\bar{a} \equiv \left(\frac{1}{N_t}\int_0^{N_t}\left(\frac{q_{jt}}{Q_t}\right)^{\eta-1}a_{jt}^{\eta-1}dj\right)^{1/(\eta-1)},
\label{eq:aggproductivity}
\end{equation}$$ a quality-weighted average of firm-level productivity. The final term, *static misallocation* $\Omega$, is a wedge that summarizes the extent to which actual labor allocations $L_{ijt}/L_{{\rm p}t}$ differ from the optimal allocation under the current blueprint distribution. Specifically, defining $A_t^* \equiv \max_{\{L_{ijt}/L_{{\rm p}t}\}}\{Y_t/L_{{\rm p}t}\}$ as the maximum level of output per labor hour, static misallocation equals the ratio $\Omega = A_t/A_t^* \in (0,1]$. As shown in Appendix 11.1.5, this wedge attains its maximum ($\Omega=1$) if and only if there is no cross-product markup heterogeneity ($\mu_{jt}=\mu$, $\forall j$). As in , heterogeneous market power distorts demand and results in a misallocation of production labor across blueprints. This is why I refer to $\Omega$ as static misallocation.[^36] Appendix 11.1.3 provides a closed-form expression for $\Omega$ in terms of the distribution of blueprint characteristics.

The aggregate production wage equals $$\begin{equation}
w_{{\rm p}t} = \Lambda\frac{Y_t}{L_{{\rm p}t}},
\label{eq:productionwage}
\end{equation}$$ where the (production) labor share of output $\Lambda = w_{{\rm p}t}L_{{\rm p}t}/Y_t$ is a sales-weighted average of product-level labor shares (inverse markups $\mu_{jt}^{-1}$):[^37] $$\begin{equation}
\Lambda = \int_0^{N_t}\frac{p_{jt}Y_{jt}}{Y_t}\mu_{jt}^{-1}dj.% = \frac{\mathcal{M}_{\eta-1}^{\eta-1}}{\mathcal{M}_\eta^\eta}.
\label{eq:labs_saleswgt}
\end{equation}$$ Recall that there are $(1-\bar{s})L_t$ production laborers who each choose how many labor hours $l_{\rm p}$ to work. Combining the labor demand curve ([\[eq:productionwage\]](#eq:productionwage)) with the labor supply curve implied by utility ([\[eq:lifetimeutility\]](#eq:lifetimeutility)), we get that labor hours per production worker equal $$\begin{equation}
l_{\rm p} = \frac{L_{{\rm p}t}}{(1-\bar{s})L_t} = \left(\frac{\Lambda}{\chi(1-\bar{s})}\right)^{\zeta/(1+\zeta)}.
\end{equation}$$ Each production worker exerts less effort if the aggregate markup $\Lambda^{-1}$ is higher, because higher markups reduce wages and therefore disincentivize work.

#### Rescaled firm's problem

A key step in solving the balanced-growth equilibrium is appropriately rescaling the equilibrium conditions. To do this, we can show that profits and firm values grow with output per capita $Y_t/L_t$. From ([\[eq:salesshare\]](#eq:salesshare)) and ([\[eq:operatingprofits\]](#eq:operatingprofits)) above, the profits of any blueprint can be rewritten[^38] $$\begin{equation}
\Pi_{ijt} = \pi_{i_1i_2\hat{q}}\frac{Y_t}{L_t}. %= \pi_{i_1i_2}e^{(\eta-1)\hat{q}}\frac{Y_t}{L_t},
\end{equation}$$ where $\pi_{i_1i_2\hat{q}}$ is a constant that depends on the current leader $i=i_1\in\{h,\ell\}$, follower $i_2\in\{h,\ell,\varnothing\}$, and relative quality $\hat{q}\equiv\log(q_{jt}/Q_t)$. From this fact, it can be shown that a firm's value is additively separable across blueprints: $$\begin{equation}
V_{it} = \sum_{j=1}^{n_{it}}V_{ijt}, \quad\textrm{where}\quad V_{ijt} = v_{i_1i_2\hat{q}}\frac{Y_t}{L_t}.
\label{eq:rescaledvalue}
\end{equation}$$ The constant $v_{i_1i_2\hat{q}}$ denotes the rescaled value of a blueprint with those characteristics. The separability in ([\[eq:rescaledvalue\]](#eq:rescaledvalue)) means that the equilibrium can be solved in terms of blueprint values $\{v_{i_1i_2\hat{q}}\}$ instead of firm values, which dramatically reduces the dimensionality of the problem. Before characterizing the solution for these blueprint values, let us derive firms' optimal R&D and M&A policies, taking blueprint values as given.

#### R&D policies

The firm allocates $X_{{\rm n}it}$ skilled laborers to new-variety creation ([\[eq:ideaprodfun_newvariety\]](#eq:ideaprodfun_newvariety)) and $X_{{\rm q}it}$ to quality innovation ([\[eq:ideaprodfun_quality\]](#eq:ideaprodfun_quality)) at the skilled wage $w_{{\rm x}t}$. The optimal number of research hours employed for new-variety creation equates the marginal cost of research with its *marginal value product*: $$\begin{equation}
w_{{\rm x}t}= \underbrace{\varepsilon(1-\alpha)\varphi (n_{it}/X_{{\rm n}it})^{1-\varepsilon}}_{\textrm{marginal product of research}}\hphantom{x} \times \underbrace{\hphantom{x_t} \bar{v}_{{\rm n}it} \hphantom{x_t}}_{\textrm{marginal value}}
\label{eq:model_solution_xrdfoc_variety}
\end{equation}$$ The blueprint values in ([\[eq:rescaledvalue\]](#eq:rescaledvalue)) and the assumption that new varieties have initial quality $q_{jt}=\lambda Q_t$ imply that the marginal value of successful new-variety innovation equals $$\begin{equation}
\bar{v}_{{\rm n}it} = \bar{v}_{{\rm n}i_1}\frac{Y_t}{L_t}, \quad\textrm{where}\quad \bar{v}_{{\rm n}i_1} = v_{i_1\varnothing,\log\lambda}.
\end{equation}$$ Likewise, the skilled wage (solved below) grows with per-capita output: $w_{{\rm x}t} = \hat{w}_{\rm x}Y_t/L_t$. These facts imply the policy $$\begin{equation}
...
\label{eq:model_solution_xrdfoc2_variety}
\end{equation}$$ Fixing the wage $\hat{w}_{\rm x}$, the firm spends relatively more on new-variety creation if the technology is more productive ($(1-\alpha)\varphi$ is high) or the resulting blueprint is more valuable ($\bar{v}_{{\rm n}i_1}$ is high).

By the exact same logic, the optimal allocation to quality innovations is $$\begin{equation}
...
\label{eq:model_solution_xrdfoc2_quality}
\end{equation}$$ where the expected value of a blueprint from successful quality innovation equals $$\begin{equation}
...
\end{equation}$$ The densities $f_{i_1i_2\hat{q}}\equiv N_{i_1i_2\hat{q}t}/N_t$ denote the share of blueprints with these characteristics; thus, in this context, $f_{i'i_2\hat{q}}$ represents the conditional probability that the firm finds and innovates on a blueprint with current leader $i_1=i'$, follower $i_2$, and relative quality $\hat{q}$.
...
\end{equation}$$ The buyer gets a fraction $\varrho$ of the surplus $v_{hi_2\hat{q}}-v_{\ell i_2\hat{q}}$ and the seller gets the remaining $1-\varrho$.

In order for this deal to take place, the high-productivity firm must first successfully find a low-productivity blueprint. The skilled wage paid to M&A search is the same as that paid to researchers, so the optimal allocation to the M&A search technology ([\[eq:ma_searchtech\]](#eq:ma_searchtech)) is $$\begin{equation}
x_{{\rm m}i_1} \equiv \frac{X_{{\rm m}it}}{n_{it}} = \left(\frac{\varepsilon\psi\bar{v}_{{\rm m}i_1}}{\hat{w}_{\rm x}}\right)^{1/(1-\varepsilon)},
\end{equation}$$ where $\bar{v}_{{\rm m}i_1}$ is the expected value of the acquired blueprint *net* of the deal price, $$\begin{equation}
...
\end{cases}
\end{equation}$$ For low-productivity firms, the expected benefit is zero, because they would never buy any blueprint that they find; these firms therefore invest nothing in the M&A search technology ($x_{{\rm m}\ell}=0$). For high-productivity firms, the expected present value is higher if it has more bargaining power ($\varrho$) and if it is more likely to find a low-productivity blueprint ($f_\ell$).

#### Skilled wages

The skilled wage $w_{{\rm x}t}$ clears the labor market by setting the demand for R&D and M&A equal to the supply of skilled labor hours. Appendix 11.3.2 shows that $w_{{\rm x}t}=\hat{w}_{\rm x}Y_t/L_t$, where $\hat{w}_{\rm x}$ is given by ([\[eq_app:optpolicy_hh_skilledwage\]](#eq_app:optpolicy_hh_skilledwage)). The rescaled wage $\hat{w}_{\rm x}$ is increasing in R&D and M&A productivity $\varphi$ and $\psi$ and in expected blueprint valuations, because these forces increase demand for skilled labor. Higher wages also increase the number of hours worked by each skilled laborer:[^39] $$\begin{equation}
l_{\rm x} = \frac{L_{xt}}{\bar{s}L_t} = \left(\frac{\hat{w}_{\rm x}}{\chi}\right)^\zeta.
\end{equation}$$ Thus, even for a fixed research population $\bar{s}L_t$, higher innovation incentives increase the equilibrium supply of research inputs through this intensive margin.

#### Economic growth

The equilibrium scales with per-capita output $y_t=Y_t/L_t$. Before deriving the growth rate, $g_y\equiv\dot{y}_t/y_t$, let us establish some notation. Define a firm's new-variety-creation intensity as $$\begin{equation}
...
\end{equation}$$ Define $\Phi_{{\rm q}i}$ and $\Psi_i$ analogously for quality innovation and M&A search, respectively. Lastly, define $\nu_i \equiv \omega_i\bar{s}L_t/N_t$ as the number of potential new type-$a_i$ entrants per existing blueprint (where, again, $\omega_h=\omega$ and $\omega_\ell=1-\omega$).

On the balanced-growth path, output per capita grows in proportion to variety $N_t^{1/(\eta-1)}$ and aggregate quality $Q_t$ (see ([\[eq:aggtfp\]](#eq:aggtfp))), so its growth rate equals[^40] $$\begin{equation}
g_y = \frac{1}{\eta-1}g_N + g_Q.
\label{eq:aggpercapitaoutputgrowth}
\end{equation}$$ The growth rate of the number of varieties equals $$\begin{equation}
g_N \equiv \frac{\dot{N}_t}{N_t} = \Phi_{{\rm n}h}(f_h+\nu_h) + \Phi_{{\rm n}\ell}(f_\ell+\nu_\ell) = g_L
\end{equation}$$ The first equality captures the fact that new-variety creation is higher when there is a higher new-variety intensity across both incumbents and entrants. The second equality is a necessary condition for balanced growth: the ratio $L_t/N_t$ is constant.[^41] The growth rate of aggregate quality equals $$\begin{equation}
g_Q \equiv \frac{\dot{Q}_t}{Q_t} = \frac{\lambda^{\eta-1}-1}{\eta-1}(g_N+\delta),
\label{eq:aggqualitygrowth}
\end{equation}$$ where $$\begin{equation}
\delta \equiv \Phi_{{\rm q}h}(f_h+\nu_h) + \Phi_{{\rm q}\ell}(f_\ell+\nu_\ell)
\end{equation}$$ is the rate of *creative destruction*, the intensity with which incumbents and entrants innovate upon existing blueprints. The rate $\delta$ is increasing in the creative destruction parameter $\alpha$. Quality growth ([\[eq:aggqualitygrowth\]](#eq:aggqualitygrowth)) is increasing in $g_N$ because new varieties have above-average quality. The larger the quality innovations $\lambda$, the faster the rate of quality growth.

#### Market valuations

The policies above depend on and determine the blueprint market values $\{v_{i_1i_2\hat{q}}\}$. Using the fact that profits are separable in quality, $\pi_{i_1i_2\hat{q}} = \pi_{i_1i_2}e^{(\eta-1)\hat{q}}$, we can further decompose the blueprint values into two parts: $$\begin{equation}
v_{i_1i_2\hat{q}} = \underbrace{v_{i_1i_2}^{\rm P}e^{(\eta-1)\hat{q}}}_{\textrm{product value}} \hphantom+ \underbrace{\hphantom{x_t} v_{i_1}^{\rm G} \hphantom{x_t}}_{\textrm{growth value}}
\end{equation}$$ The product value represents the present value of profits directly generated by the blueprint; the growth value represents the contribution of the blueprint to generating future profits via R&D and M&A. Because we are in a riskless economy, every firm is discounted by the riskfree rate, $$\begin{equation}
r_f = \rho + g_y.
\end{equation}$$ The coefficients $\{v_{i_1i_2}^{\rm P},v_{i_1}^{\rm G}\}$ are characterized by a system of Hamilton-Jacobi-Bellman (HJB) equations. The HJBs for product values are $$\begin{equation}
\underbrace{\rho v_{i_1i_2}^{\rm P}}_{\textrm{discount}} = \underbrace{\pi_{i_1i_2}}_{\textrm{profits}} + \underbrace{\mathbb{I}_{i_1=\ell}\Psi_hf_h(1-\varrho)(v_{hi_2}^{\rm P}-v_{\ell i_2}^{\rm P})}_{\textrm{M\&A exit}} - \underbrace{\hphantom{x} \delta v_{i_1i_2}^{\rm P} \hphantom{x}}_{\textrm{creat. destr.}} - \underbrace{(\eta-1)g_Qv_{i_1i_2}^{\rm P}}_{\textrm{quality depr.}}.
\label{eq:hjb_products}
\end{equation}$$ Product values are increasing in profits and, for low-productivity firms, the possibility of being acquired at a premium; they are decreasing in the risk of creative destruction $\delta$ and in the rate of relative quality depreciation $(\eta-1)g_Q$. The HJBs for growth values are $$\begin{multline}
...
+ \underbrace{\mathbb{I}_{i_1=\ell}\Psi_hf_h(1-\varrho)(v_h^{\rm G}-v_\ell^{\rm G})}_{\textrm{M\&A exit}} - \underbrace{\hphantom{x} \delta v_{i_1}^{\rm G} \hphantom{x}}_{\textrm{creat. destr.}}
\label{eq:hjb_growthopps}
...
The KFEs follow from the fact that the net flow of blueprints equals $$\begin{equation}
\dot{N}_{i_1i_2\hat{q}t} = N_t\dot{f}_{i_1i_2\hat{q}t} + \dot{N}_tf_{i_1i_2\hat{q}t} = g_NN_tf_{i_1i_2\hat{q}},
\end{equation}$$ where the second equality imposes a stationary distribution. This net flow equals the sum of constituent flows from R&D and M&A. For monopoly products ($i_2=\varnothing$), the KFEs are $$\begin{equation}
\underbrace{g_Nf_{i_1\varnothing\hat{q}}}_{\textrm{net flow}} = \underbrace{[\partial_{\hat{q}}f_{i_1\varnothing\hat{q}}]g_Q}_{\textrm{quality depr.}} + \underbrace{\Phi_{{\rm n}i_1}(f_{i_1}+\nu_{i_1})\mathbb{I}_{\hat{q}=\log\lambda}}_{\textrm{new varieties}}+ \underbrace{(\mathbb{I}_{i_1=h}-\mathbb{I}_{i_1=\ell})\Psi_hf_hf_{\ell\varnothing\hat{q}}}_{\textrm{acquisitions}} - \underbrace{\hphantom{x} \delta f_{i_1\varnothing\hat{q}} \hphantom{x}}_{\textrm{creat.~destr.}}.
\label{eq:kfe_monopprods}
\end{equation}$$ Blueprints for monopoly products flow in and out due to quality depreciation, new-variety creation, and creative destruction. The term captioned "acquisitions\" represents the flows from high-productivity firms buying low-productivity monopoly products---note the asymmetry in $i_1$. The KFEs for competitive markets ($i_2\in\{h,\ell\}$) are $$\begin{multline}
...
+ \underbrace{(\mathbb{I}_{i_1=h}-\mathbb{I}_{i_1=\ell})\Psi_hf_hf_{\ell i_2\hat{q}}}_{\textrm{acquisitions}} - \underbrace{\hphantom{x} \delta f_{i_1i_2\hat{q}} \hphantom{x}}_{\textrm{creat.~destr.}}.
\label{eq:kfe_competitiveprods}
\end{multline}$$ The only difference from ([\[eq:kfe_monopprods\]](#eq:kfe_monopprods)) is that, instead of inflows from new varieties, there are inflows from quality innovations on incumbent firms' blueprints.

The total type-$a_{i_1}$ blueprint share $f_{i_1}=N_{i_1t}/N_t$ is the main determinant of total revenue and value shares in the economy. Aggregating the KFEs across $i_2$ and $\hat{q}$ implies that the total type-$a_{i_1}$ blueprint share satisfies $$\begin{equation}
...
\label{eq:kfe_leader}
\end{equation}$$ This aggregated KFE displays the key forces driving the total high-productivity blueprint share $f_h$. On the one hand, $f_h$ will be driven up by the fact that these firms do more R&D ($\Phi_h>\Phi_\ell$) and that M&A is asymmetric (the last term of ([\[eq:kfe_leader\]](#eq:kfe_leader))). On the other hand, $f_h$ will be lower if fewer entrepreneurs are high-productivity to begin with ($\omega_h<\omega_\ell$, so $\nu_h<\nu_\ell$).
...
1.  *High-productivity producers earn higher revenues, higher markups, and higher profits: $$\begin{equation}
...
Before estimating the model, I calibrate eight parameters externally. Two come directly from data. The population growth rate $g_L$ is $1.25\%$, the U.S. average from 1955--1985. The share of skilled workers in the labor force is $14.2\%$, which is the share of managers, engineers, and scientists computed by .

The elasticity of substitution between goods $\eta$ is set equal to $2.9$, the median estimate of . Households have log utility, meaning that they have a unit elasticity of intertemporal substitution (EIS). Because none of the moments of interest will depend on the labor disutility scalar $\chi$, I normalize it to 1. For the Frisch elasticity of labor supply, I assume a value of 0.5, in the range of most empirical estimates from the labor literature (see and for summaries).

The elasticity $\varepsilon\in(0,1)$, which determines the relative importance of laborers versus existing ideas in the idea-production functions, is common in endogenous-growth models. Following , I set it to 0.5, in line with a large body of evidence from the microeconomic innovation literature (**blundell2002individual?**; **hall2001patent?**; **hall1993tax?**; **bloom2002tax?**; **wilson2009beggar?**).[^42] Finally, I assume buyers and sellers have equal bargaining power in M&A negotiations ($\varrho=0.5$).[^43]

#### Estimated parameters

In the first stage of the estimation, I estimate seven model parameters in the initial balanced-growth path using micro and macro data: $$\begin{equation}
\Theta = \biggr[ \underbrace{\phantom{a/} \rho \phantom{a/}}_{\textrm{preferences}}  \underbrace{ \phantom{a/} \varphi_0 \phantom{xx.x}  \lambda \phantom{xx.x} \alpha \hphantom{a/}}_{\textrm{innovation}}  \underbrace{\phantom{a/} \psi \phantom{a/}}_{\textrm{acquisition}}  \underbrace{\phantom{a/} a_h/a_\ell \phantom{xx.x} \omega \phantom{a/}}_{\textrm{heterogeneity}} \biggl].
\label{eq:estparams_bge_theta}
\end{equation}$$ Table [\[tab:estimparams\]](#tab:estimparams) also lists these parameters with their descriptions. Note that the subscript on innovation productivity $\varphi_0$ indicates that this is the constant, pre-transition value.
...
& & & & (0.0284) & & & & (0.0303) & &\
High-prod. entrep. share & & $\omega$ & & 0.0124 & & & 0.5463 & & 0.5463\
& & & & (0.0051) & & & & (0.0162) & &\
...
Larger quality-innovation increments $\lambda>1$ will mean that aggregate quality $Q_t$ grows faster with each new idea. Hence, $\lambda$ can be identified from the growth rate of aggregate output per capita $$\begin{equation}
g_y = \frac{\lambda^{\eta-1}}{\eta-1}g_L + \frac{\lambda^{\eta-1}-1}{\eta-1}\delta.
\end{equation}$$ In the data, I use percentage growth in real output per capita from 1950--1985.
...
Let $V_{{\rm m}t}$ denote aggregate M&A deal flow: the total dollar amount spent on completed acquisitions of firms (not on M&A search labor). We can identify $\psi$ from the aggregate M&A-to-value ratio: $$\begin{equation}
...
\end{equation}$$ This ratio has two components. The first is the frequency with which targets are found and acquired, which is higher when $\psi$ (and hence $\Psi_h$) is higher. The second is the ratio of the average value of an acquired blueprint $\bar{v}_{\rm m}$ to the average value of an existing blueprint $V_t/N_t$. By scaling by aggregate value, we control for blueprint-level valuation changes (the second component), isolating the frequency component of M&A.
...
\end{equation}$$ By the law of total variance, this sales-weighted variance can be decomposed across types as $$\begin{equation}
...
Empirically, these sales-weighted variances are computed across firms in Compustat from 1965--1975. To standardize units, the moments reported in Table [\[tab:estimparams\]](#tab:estimparams) are standard deviations, scaled by the respective aggregate ratios (see Appendix 14.2 for further discussion).

#### Discussion of estimation results

Table [\[tab:estimparams\]](#tab:estimparams) reports the estimation results. The model matches each moment well (of course, it is exactly identified), and the relatively small standard errors validate parameter identification. Importantly, the estimate of $\varphi_0$ is very precise, suggesting that R&D-to-value is a good identifying moment (more on this in 4.3). Appendix 14.3 presents additional corroborating evidence of identification.
...
As in the balanced-growth path, innovation productivity is identified by the aggregate R&D-to-value ratio. The intuition is as follows. As we saw in the first-order conditions ([\[eq:model_solution_xrdfoc2_variety\]](#eq:model_solution_xrdfoc2_variety)) and ([\[eq:model_solution_xrdfoc2_quality\]](#eq:model_solution_xrdfoc2_quality)), a firm's R&D spending is increasing in innovation productivity $\varphi$, increasing in the present value of new ideas $\bar{v}_{{\rm n}i}$ and $\bar{v}_{{\rm q}i}$, and decreasing in the skilled wage $w_{{\rm x}t}$. The main challenge to identifying $\varphi$ from R&D spending is controlling for valuation and wage changes. The insight for identification is that one can do this by dividing by aggregate value.

It is most straightforward to see this in the case with inelastic labor supply ($\zeta=0$). Letting $X_{{\rm r}t} \equiv X_{{\rm n}t} + X_{{\rm q}t}$ denote the aggregate quantity of researchers employed by incumbents, the equilibrium R&D-to-value ratio at any point in time equals[^48] $$\begin{equation}
...
\label{eq:aggxrdval_identification}
\end{equation}$$ where the average present value of a new idea from R&D equals $$\begin{equation}
...
<div class="center">
<p><span>-2cm</span> <img src="paperplots/trans_growthrate_bea" style="width:70.0%" alt="image" /></p>
</div>
...
<div class="center">
<p><span>-2cm</span> <img src="paperplots/trans_valsale_agg" style="width:70.0%" alt="image" /></p>
</div>
...
<div class="center">
<p><span>-2cm</span> <img src="paperplots/trans_maxrd_sm" style="width:70.0%" alt="image" /></p>
</div>
...
<div class="center">
<p><span>-2cm</span> <img src="paperplots/trans_entry" style="width:70.0%" alt="image" /></p>
</div>
...
To state this formally, note that the value function of the representative household equals $$\begin{equation}
U_t = k(t) + \rho^{-1}\log\left(\frac{\overline{W}_t}{L_t}\right),
\end{equation}$$ where $k(t)$ is a deterministic function of time and $\overline{W}_t/L_t$ is *total wealth* per person.[^60] Total wealth is the present value of per-capita consumption $c_t$: $$\begin{equation}
\frac{\overline{W}_t}{L_t} = \mathbb{E}_t\left[\int_0^\infty\frac{\xi_{t+\tau}}{\xi_t}c_{t+\tau}d\tau\right] = \rho^{-1}c_t.
\end{equation}$$ Because consumption equals output in equilibrium, we therefore have $$\begin{equation}
\overline{W}_t = \rho^{-1}Y_t.
\label{eq:totalwealth_output}
\end{equation}$$ Total wealth is always exactly proportional to output, even along the transition path. Moreover, because output is the sum of all dividends and labor income, $$\begin{equation}
Y_t = D_t + w_{{\rm p}t}L_{{\rm p}t} + w_{{\rm x}t}X_t,
\end{equation}$$ we can decompose total wealth into three components: $$\begin{equation}
\overline{W}_t = V_t + \textrm{PV}_t^{\rm entrants} + \textrm{PV}_t^{\rm labor}.
\label{eq:totalwealth_decomp}
\end{equation}$$ That is, total wealth includes not only the value $V_t$ of the stock market (the claim to current incumbent firms' dividends), but also the present value $\textrm{PV}_t^{\rm entrants}$ of future entrants (firms not yet traded) and the present value $\textrm{PV}_t^{\rm labor}$ of labor income paid out to households.

<figure id="fig:welfare_wealthcomp" data-latex-placement="t">
<div class="center">
<p><span>-2cm</span> <img src="paperplots/trans_wealthgrowth" style="width:70.0%" alt="image" /></p>
</div>
...
This means that, while the value of the stock market boomed relative to output (see Figure 7), the present value of consumption ([\[eq:totalwealth_output\]](#eq:totalwealth_output)) stagnated with output. Figure 12 plots the cumulative change in the log of each of these wealth measures ($V_t$ and $\overline{W}_t$) since 1975. As output growth fell, total wealth stagnated; at the same time, the value of the stock market accelerated. The decomposition ([\[eq:totalwealth_decomp\]](#eq:totalwealth_decomp)) accounts for this: all of the gains in stock-market wealth came at the expense of labor income and future entrants. The present value of labor income falls mainly because high-productivity firms have higher markups. The present value of entrants falls both because the rate of new-firm entry falls (see Figure 10) and because the average value of incumbent firms (which tend increasingly to be high-productivity) rises relative to the average value of entrants (which tend to be low-productivity).
...
Perhaps the most unrealistic assumption about the baseline model is that there is no systematic risk, so firms are discounted at the riskless interest rate. We can generalize this along two dimension, following and . First, let us generalize firms' production functions ([\[eq:model_prodfun\]](#eq:model_prodfun)) to include a common, *aggregate* productivity shock $z_t$, which follows a jump-diffusion: $$\begin{equation}
Y_{ijt} = a_ie^{z_t}L_{ijt}, \quad\text{where}\quad dz_t = \kappa_z(\bar{z}-z_t)dt + \sigma_zdB_t - \zeta_zdJ_t,
\label{eq:prodfun_zshock}
\end{equation}$$ for $B_t$ a Brownian motion and $J_t$ a Poisson process with intensity $p_z$. For small $p_z$ and $\zeta_z>0$, this jump term represents a rare disaster, and makes it possible to match a sizable equity premium.[^61] Second, let us generalize household preferences ([\[eq:lifetimeutility\]](#eq:lifetimeutility)) to the recursive formulation of , which continues to assume a unit EIS and a constant Frisch elasticity, but allows for potentially higher risk aversion $\gamma\geq1$.[^62]

Appendix 15.1 re-solves the model under these assumptions and shows that the equilibrium conditions are *exactly the same* as in the case without systematic risk, except now the discount factor $\rho$ in the HJBs ([\[eq:hjb_products\]](#eq:hjb_products)) and ([\[eq:hjb_growthopps\]](#eq:hjb_growthopps)) is replaced by a *risk-adjusted* discount factor $$\begin{equation}
...
\end{equation}$$ The risk adjustment incorporates both the equity risk premium, which raises $\rho^*$, and the precautionary savings motive, which lowers $\rho^*$. Because the model estimation identified the discount factor using a valuation ratio (not the riskfree rate), the estimate is still appropriate in this setting, but should be reinterpreted as an estimate of $\rho^*$, not of $\rho$.

#### Physical capital

Another plausible generalization of the production function ([\[eq:model_prodfun\]](#eq:model_prodfun)) is to assume that it takes both labor and physical capital $K_t$ as inputs: $$\begin{equation}
Y_{ijt} = a_iK_{ijt}^\beta L_{ijt}^{1-\beta}.
\end{equation}$$ For simplicity, suppose that the capital stock depreciates at constant rate $\delta_K$ and is produced by a competitive capital-goods sector that makes total investment $I_t$ and rents capital out to goods producers at an endogenous rental rate $r_{Kt}$.[^63] This implies total capital accumulation $$\begin{equation}
\dot{K}_t = I_t - \delta_KK_t.
\end{equation}$$ Appendix 15.2 derives two results. First, the equilibrium conditions are all the same, except the "labor share\" $\Lambda$ from ([\[eq:labs_saleswgt\]](#eq:labs_saleswgt)) is reinterpreted as the total factor share, consisting of the labor share $(1-\beta)\Lambda$ and the capital share $\beta\Lambda$. Because the estimation moments depend only on the profit share ($1-\Lambda$), the estimates remain valid. The second implication is that the equilibrium investment-capital ratio is given by $$\begin{equation}
\frac{I_t}{K_t} = \delta_K + g_L + g_{yt} + g_{\Lambda t}.
\end{equation}$$ Hence, the decline in per-capita output growth $g_{yt}$ and the slight rise in the aggregate markup $\Lambda^{-1}$ in the model imply a decline in physical investment. This is a well-documented trend in the data and the subject of several studies (**farhi2018accounting?**; **miller2025sovereign?**; **crouzet2023rents?**). In the model, investment stagnates because "average $Q$\" booms but the "marginal $q$\" of capital investment remains constant. Intuitively, capital exists to scale up production of new ideas, so fewer new ideas means less need for capital.
...
As explains, assuming that firms remain high-productivity indefinitely implies that the average age of large firms is too high. also show that younger firms tend to have higher R&D intensities and growth rates. and address these facts by assuming that high-productivity firms switch permanently to low-productivity firms at some random point in the life cycle.
...
This paper's findings concern trends over the last half-century; the next half-century may look different. For example, recent work has considered that the development of artificial intelligence (AI) may precipitate a revival of innovation productivity, if AI can replace researchers in generating new ideas (**aghion2019artificial?**; **jones2026past?**). The framework in this paper is well-suited for thinking about what such a transformation could mean for the economy in the future.
...
Public and private acquirers are identified by the variable `APUBLIC `$\in$` {"Public","Priv."}`, and targets are likewise identified by `TPUBLIC`. To merge SDC into CRSP/Compustat at the acquirer level, I use the acquirer identifier mappings (`agvkey` in SDC matched to `gvkey`) provided by , who build on the work of . The target's market value before a merger announcement, which I use to compute the M&A premium, is given by the variable `MV`.

## Other data sources 

For analysis of trends in patent value, I use the estimates from , updated to 2020. In analyses of public versus private firm R&D intensity, I use the NSF R&D-to-GDP ratio for R&D performed by businesses.[^64] The specific series comes from Table 1 of the National Patterns of R&D Resources (annual series) from the National Center for Science and Engineering Statistics. Data on firm entry and the employment distribution come from the Business Dynamics Statistics (BDS). Finally, per-capita real output and population growth data come from the BEA.
...
Under this notation, we have $\mathcal{I}_t=\mathcal{I}_t^{\rm ent}\cup\mathcal{I}_t^{\rm cont}$, so the time-$t$ aggregate value is $$\begin{equation}
Z_t = \sum_{i\in\mathcal{I}_t^{\rm ent}}s_{it}Z_{it} + \sum_{i\in\mathcal{I}_t^{\rm cont}}s_{it}Z_{it}.
\end{equation}$$ Likewise, $\mathcal{I}_{t-1}=\mathcal{I}_t^{\rm exit}\cup\mathcal{I}_t^{\rm cont}$, so the lagged aggregate can be written $$\begin{equation}
Z_{t-1} = \sum_{i\in\mathcal{I}_t^{\rm exit}}s_{i,t-1}Z_{i,t-1} + \sum_{i\in\mathcal{I}_t^{\rm cont}}s_{i,t-1}Z_{i,t-1}.
\end{equation}$$ Differencing these equations, we have that the total change equals $$\begin{equation}
...
\end{equation}$$ Adding and subtracting $s_t^{\rm ent}Z_t^{\rm cont}$ and $s_{t-1}^{\rm exit}Z_{t-1}^{\rm cont}$ on the right-hand side (and recalling that $s_t^{\rm ent}=1-s_t^{\rm cont}$ and $s_{t-1}^{\rm exit}=1-s_{t-1}^{\rm cont}$) then implies $$\begin{equation}
...
\end{equation}$$ The last term can be decomposed as $$\begin{align}
...
\end{align}$$ Using the index notation $(\overline{\phantom{}\cdot\vphantom{|}\vphantom{}})$ from ([\[eq:valsale_tornqvist\]](#eq:valsale_tornqvist)), we therefore have the total decomposition $$\begin{multline}
...
\end{multline}$$ If we redefine the lags and leads of entrants as described in the main text, then this decomposition can be written more concisely in the form ([\[eq:valsale_decomp\]](#eq:valsale_decomp)) and ([\[eq:cfval_decomp\]](#eq:cfval_decomp)).

### Relation to other macro decompositions 

The decomposition I construct is closely related to a larger class of macroeconomic decompositions. In the notation above, the decomposition has components $$\begin{multline}
...
\label{eq_app:decomp_haltiwanger}
\end{multline}$$ and use this decomposition as a slight adjustment to that of . Conventionally, these decompositions have been used to understand the components of productivity growth from plant-level data (**foster2001aggregate?**; **bartelsman2000understanding?**; **lentz2008empirical?**). More recently, the same decomposition has been used to understand the drivers of secular macro trends. For example, use it to decompose the rise in the sales-weighted aggregate markup.

While this decomposition may be ideal for studying the drivers of productivity growth, it is not suitable for studying secular change in aggregate valuations for two reasons. First, the cross term is mechanically very negative, because transitory shocks to sales shares tend to be highly negatively correlated with transitory shocks to value-to-sales ratios (through the denominator).[^70] The solution to this is to use the indices $(\overline{\phantom{}\cdot\vphantom{|}\vphantom{}})$ for continuers. Without entry and exit, this indexing is equivalent to adding half of the cross term to the within effect and the other half to the between effect:[^71] $$\begin{align}
...
Suppose there is no entry or exit, and the set of firms are ex ante homogeneous. Firm $i$ has value $V_{it}$ and sales $Y_{it}$, where $$\begin{equation}
Y_{it} = \bar{Y}\epsilon_{{\rm Y}it} \quad\textrm{and}\quad V_{it} = \bar{V}\epsilon_{{\rm V}it},
\end{equation}$$ where the shocks $\epsilon_{{\rm Y}it}$ and $\epsilon_{{\rm V}it}$ are independent over time and of each other---that is, they are completely transitory. Without loss of generality, assume that $\mathbb{E}[\epsilon_{{\rm Y}it}] = \mathbb{E}[\epsilon_{{\rm V}it}] = 1$.

The aggregate value-to-sales ratio is then a constant $$\begin{equation}
\frac{V_t}{Y_t} = \frac{\bar{V}}{\bar{Y}}.
\end{equation}$$ Now let us consider what happens to the decomposition terms. The valuation ratio and sales share of firm $i$ are $$\begin{equation}
...
\end{equation}$$ Hence, the Haltiwanger terms from $t-1$ to $t$ equal $$\begin{align}
...
The indexing adjustment corrects this bias: $$\begin{equation}
\textrm{within}_{\rm H} + \frac{1}{2}\textrm{cross}_{\rm H} = \textrm{between}_{\rm H} + \frac{1}{2}\textrm{cross}_{\rm H} = 0.
\end{equation}$$ Consistent with a stationary firm distribution, both the within-firm change and compositional change are now zero in this economy.

#### Example 2: Persistent idiosyncratic shocks

Consider the setting in the previous example, but now, instead of independent transitory shocks, the numerator and denominator are subject to (potentially correlated) persistent shocks. Specifically, the log sales of firm $i$, denoted $y_{it}\equiv\log Y_{it}$, follow the autoregression $$\begin{equation}
y_{it} = \log\bar{Y} + \phi_yy_{i,t-1} + \epsilon_{yit}, % - \frac{1}{2}\frac{\sigma_y^2}{1-\phi_y^2}
\end{equation}$$ where $\phi_y\leq1$ is the persistence of the shocks. Similarly, the log value-to-sales ratio, $v_{it}\equiv\log(V_{it}/Y_{it})$, follows the autoregression[^72] $$\begin{equation}
v_{it} = \bar{v} + \phi_vv_{i,t-1} + \epsilon_{vit}.
\end{equation}$$ The shocks $[\epsilon_{yit},\epsilon_{vit}]$ are iid over time and, at each time $t$, jointly normal with variances $\sigma_y^2$ and $\sigma_x^2$ and covariance $\sigma_{xy}$. It is straightforward to compute the aggregate value-to-sales ratio $V_t/Y_t$ and see that it is a constant.
...
\end{align}$$ By the same algebra, one can see that $\mathbb{E}\left[e^{y_{i,t-1}+v_{it}}\right]$ is the exact same. Hence, the total compositional change (and, in turn, the within-firm change) is zero: $$\begin{equation}
\textrm{between}_{\rm H} + \frac{1}{2}\textrm{cross}_{\rm H} = \textrm{within}_{\rm H} + \frac{1}{2}\textrm{cross}_{\rm H} = 0.
\end{equation}$$ Again, the indexing corrects the bias that would be present in the individual terms.

#### Example 3: Life-cycle trends and entry

This final example demonstrates how adding net entry to the within-firm change corrects for life-cycle trends in firm valuations. Consider an economy with a mass of firms producing aggregate output $Y_t$. At each time $t$, a mass of new entrants begins producing; the total output of this cohort is proportional to last period's total output: $$\begin{equation}
Y_t^{(0)} = \theta Y_{t-1}.
\end{equation}$$ All other firms continue producing the same amount as before, so it follows that $$\begin{equation}
Y_t = (1+\theta)Y_{t-1}.
\end{equation}$$ This means that the total sales share of firms aged $\tau$ equals $$\begin{equation}
\frac{Y_t^{(\tau)}}{Y_t} = \frac{\theta}{1+\theta}(1+\theta)^{-\tau}.
\end{equation}$$ Furthermore, assume that every entering firm has the same value-to-sales ratio $$\begin{equation}
\frac{V_{it}}{Y_{it}} = v^{(0)} = \bar{v}.
\end{equation}$$ As firms age, their value-to-sales ratios fall by a factor of $(1+\delta)^{-1}$: for a firm of age $\tau$, the valuation is therefore $$\begin{equation}
\frac{V_{it}}{Y_{it}} = v^{(\tau)} = \bar{v}(1+\delta)^{-\tau}.
\end{equation}$$ All of these facts together imply that the aggregate value-to-sales ratio equals the constant $$\begin{equation}
\frac{V_t}{Y_t} = \sum_{\tau=0}^\infty \frac{Y_t^{(\tau)}}{Y_t}v^{(\tau)} = \bar{v}\frac{\theta(1+\delta)}{\theta(1+\delta)+\delta}.
\end{equation}$$ The entrants offset the declining valuations of aging firms, yielding a stationary distribution.
...
The adjusted decomposition makes two important adjustments to the above terms. First, the sales shares among continuers are defined relative to total continuer output ($Y_t^{\rm cont} = \sum_{\tau=1}Y_t^{(\tau)}$), not aggregate output ($Y_t = Y_t^{\rm cont} + Y_t^{(0)})$. Consequently, the "between\" term above is no longer negative, but zero: $$\begin{equation}
\textrm{between}_{\rm H} = \sum_{\tau=0}^\infty \left(\frac{Y_t^{(\tau+1)}}{Y_t^{\rm cont}} - \frac{Y_{t-1}^{(\tau)}}{Y_{t-1}}\right)v^{(\tau)} = 0,
\end{equation}$$ and similarly for the "cross\" term. Second, by adding the entry term to the within-firm term, we offset the deterministic life-cycle trend in valuations, so that it is now also zero: $$\begin{equation}
\textrm{entry}_{\rm H} + \textrm{within}_{\rm H} = 0.
\end{equation}$$ This example works similarly if we allow firm death and add the resulting exit term to the within-firm component.
...
<div class="center">
<p><span>-2cm</span> <img src="paperplots/decomp_wide_xrd_sale" style="width:80.0%" alt="image" /></p>
</div>
...
Figure 20 plots the decompositions ([\[eq:valsale_decomp\]](#eq:valsale_decomp)) and ([\[eq:cfval_decomp\]](#eq:cfval_decomp)) at the industry level instead of the firm level. Panels A and B use NAICS 2-digit industry definitions and C and D use NAICS 3-digit industries. In all plots, the amount of reallocation is much smaller than at the firm level, suggesting that most of the reallocation was a within-industry phenomenon.
...
As discussed in the main text, we can exploit cross-industry variation over time to test whether the key macro facts are related in the cross-section. The first prediction I test is whether those industries with the largest declines in R&D-to-value had the largest increase in market valuations and M&A intensity. Table [\[tab:xrd_val_indreg\]](#tab:xrd_val_indreg) reports the results for four regressions for NAICS-5 industries. The first specification regresses the total (log) change in an industry value-to-sales ratio from 1980 to 2020 on the total change in the industry's R&D-to-value ratio over the same period. The negative coefficient estimate implies that those sectors with the largest decline in R&D-to-value had the largest increase in total value-to-sales ratios. Similarly, the other three regressions imply that falling R&D-to-value is associated with rising value-to-cashflows, rising M&A-to-R&D, and rising M&A-to-value. These findings corroborate the first prediction.

-2cm

to 1 Dependent variable: & & $\Delta\log\left(\dfrac{\textrm{value}_k}{\textrm{sales}_k}\right)$ & & $\Delta\log\left(\dfrac{\textrm{value}_k}{\textrm{cashflow}_k}\right)$ & & $\Delta\log\left(\dfrac{\textrm{M\&A}_k}{\textrm{R\&D}_k}\right)$ & & $\Delta\log\left(\dfrac{\textrm{M\&A}_k}{\textrm{value}_k}\right)$\
& & (1) & & (2) & & (3) & & (4)\
...
Consider any individual household in the economy. The household chooses quantities $\{y_{ijt}\}$ of each good in order to maximize the value of its total consumption ([\[eq:cesdemand\]](#eq:cesdemand)) and ([\[eq:demand_withinvariety\]](#eq:demand_withinvariety)). The household takes the prices $\{p_{ijt}\}$ of these goods as given. Let $W_{ct}$ denote the total expenditure of the household on all consumption goods (to be determined in the household's dynamic consumption-saving problem). Its budget constraint is then $$\begin{equation}
W_{ct} \geq \int_0^{N_t}\sum_{i=1}^{m_{jt}}p_{ijt}y_{ijt}dj.
\end{equation}$$ If we define $\hat{y}_{ij}\equiv q_{ij}y_{ij}$ and $\hat{p}_{ij}\equiv p_{ij}/q_{ij}$, then this optimization solves the static Lagrangian $$\begin{equation}
...
\end{equation}$$ where $c(\{\hat{y}_{ijt}\})$ nests ([\[eq:cesdemand\]](#eq:cesdemand)) and ([\[eq:demand_withinvariety\]](#eq:demand_withinvariety)). Stated this way, the problem is of the same form as the standard CES optimization without quality differences. The first-order conditions are $$\begin{equation}
\left(\frac{y_{jt}}{c_t}\right)^{-1/\eta} = \lambda_{ct}\hat{p}_{ijt}.
\label{eq:foc_hhdemand_1}
\end{equation}$$ This implies that, within product market $j$, firms must equate quality-adjusted prices: $$\begin{equation}
\hat{p}_{ijt} = \hat{p}_{i'jt},
\label{eq_app:foc_phat}
\end{equation}$$ for all $i$ and $i'$ producing in the market. Denote by $p_j=\hat{p}_{ij}$ the price index for the whole product market, and note that $p_jy_j = \sum_{i=1}^{m_j}p_{ij}y_{ij}$.

Given this price index, the remainder of the solution follows standard CES algebra. First, the first-order condition ([\[eq:foc_hhdemand_1\]](#eq:foc_hhdemand_1)) implies that, for any goods $j$ and $j'$, $$\begin{equation}
p_{jt}^\eta y_{jt} = p_{j't}^\eta y_{j't}.
\end{equation}$$ This means that the household's total expense on consumption is $$\begin{equation}
\int_0^{N_t}p_{j't}y_{j't}dj' = p_{jt}^\eta y_{jt}\int_0^{N_t}p_{j't}^{1-\eta}dj'.
\end{equation}$$ Now define the aggregate price index $p_t$ such that $p_tc_t=\sum_jp_{jt}y_{jt}$. Substituting this into the left-hand side of the last equation, then aggregating the $y_{jt}$ up to $c_t$ implies $$\begin{equation}
p_t =\left(\int_0^{N_t}p_{jt}^{1-\eta}dj\right)^{1/(1-\eta)}.
\end{equation}$$ This implies that good $j$'s price satisfies $$\begin{equation}
...
\label{eq_app:foc_hhdemand_agg}
\end{equation}$$ for every firm $i$ in variety $j$.

This is the solution for a single household; the final step is to aggregate across households. Note that the first-order condition ([\[eq:foc_hhdemand_1\]](#eq:foc_hhdemand_1)) aggregates across households to $$\begin{equation}
\left(\frac{Y_{jt}}{Y_t}\right)^{-1/\eta} = \lambda_{ct}\hat{p}_{ijt},
\end{equation}$$ where $Y_{jt}$ is aggregate consumption of good $j$ and $Y_t$ is aggregate consumption of all goods. Thus, all of the steps above hold in terms of aggregates, implying the aggregate demand curve $$\begin{equation}
p_{ijt} = q_{ijt}\left(\frac{Y_{jt}}{Y_t}\right)^{-1/\eta}p_t.
\end{equation}$$ Aggregate consumption is the numeraire, so we normalize $p_t=1$.

### Firm profit maximization 

The firm's profits, and hence its pricing and production decisions, are separable across products, so it suffices to consider the profit maximization for a single product. Firms compete à la Bertrand, taking as given the demand curves ([\[eq_app:foc_hhdemand_agg\]](#eq_app:foc_hhdemand_agg)) to determine their output. A standard result in Bertrand competition is that firms may be at a corner solution if competition is sufficiently intense. Substituting the demand curve into firm $i$'s profits for product $j$ implies profits $$\begin{align}
...
\end{align}$$ The second equality imposes the constraint ([\[eq_app:foc_phat\]](#eq_app:foc_phat)). The firm chooses its (quality-adjusted) price $p_{jt}=p_{ijt}/q_{ijt}$ given the qualities and quantities $\{q_{-ijt},Y_{-ijt}\}$ of competitors.

First, let us consider interior solutions: the first-order condition with respect to $p_{jt}$ is $$\begin{equation}
(1-\eta)p_{jt}^{-\eta} + \eta\frac{w_{{\rm p}t}}{a_iq_{ijt}}p_{jt}^{-\eta-1} = 0.
\end{equation}$$ (Note that $Y_t-\sum_{-i\neq i}q_{-ijt}Y_{-ijt}=q_{ijt}Y_{ijt}>0$ in an interior solution). This implies $$\begin{equation}
p_{ijt} = q_{ijt}p_{jt} = \frac{\eta}{\eta-1}\frac{w_{{\rm p}t}}{a_i}.
\end{equation}$$ Defining the firm's markup as its price relative to marginal cost, we have $$\begin{equation}
\mu_{ijt} \equiv \frac{p_{ijt}}{w_{{\rm p}t}/a_i} = \frac{\eta}{\eta-1},
\end{equation}$$ the usual CES monopoly markup.

This interior solution will not be a Nash equilibrium if it is possible for a competitor to lower its price while still making a profit. Define a firm's quality-adjusted marginal cost as $$\begin{equation}
mc_{ijt} \equiv \frac{w_{{\rm p}t}}{a_iq_{ijt}}.
\label{eq_app:markup_def}
\end{equation}$$ Now consider the two firms with the lowest marginal costs; denote by $i_1$ the firm with the lowest marginal cost (the "leader\") and by $i_2$ that with the second lowest (the "follower\"). Because quality-adjusted prices are equated through $p_j=p_{ij}/q_{ij}$, the markup of firm $i_2$ can be equivalently expressed as: $$\begin{equation}
\mu_{i_2jt} = \frac{p_{jt}}{mc_{i_2jt}} = \frac{p_{i_1jt}}{q_{i_1jt}mc_{i_2jt}}.
\end{equation}$$ The lowest price at which firm $i_1$ can guarantee firm $i_2$ will be unwilling to further reduce prices is that for which $\mu_{i_2jt}=1$, or $$\begin{equation}
p_{i_1jt} = q_{i_1jt}mc_{i_2jt}.
\end{equation}$$ At this price, the follower earns zero profits and the markup of the leader is $$\begin{equation}
\mu_{i_1jt} = \frac{q_{i_1jt}mc_{i_2jt}}{w_{{\rm p}t}/a_i} = \frac{mc_{i_2jt}}{mc_{i_1jt}} = \frac{a_{i_1}q_{i_1jt}}{a_{i_2}q_{i_2jt}}.
\end{equation}$$ Combining these two solutions, the leader's markup equals $$\begin{equation}
\mu_{ijt} = \min\left\{\frac{a_{i_1}q_{i_1jt}}{a_{i_2}q_{i_2jt}},\frac{\eta}{\eta-1}\right\}
\end{equation}$$ and the follower always has a markup of one. Because we have assumed that firms earning zero markups choose not to produce (Section 2), product $j$ is only produced by the leader.

The last piece is to determine the set of possible market structures and resulting markups. If a firm creates a new variety, it obviously has no follower $(i_2=\varnothing)$. Alternatively, a firm could enter an existing market by innovating on an existing blueprint or by buying the leader's blueprint. In either of these cases, the quality gap is $q_{i_1}/q_{i_2} = \lambda$. Hence, we have six potential market structures, which can be summarized by the pair $\{i_1,i_2\}\in\{h,\ell\}\times\{h,\ell,\varnothing\}$. Denoting by $\mu_{i_1i_2}$ the markup given this pair, the results above imply that monopolists get the CES markup, $$\begin{equation}
\mu_{h\varnothing}=\mu_{\ell\varnothing}=\frac{\eta}{\eta-1},
\end{equation}$$ same-type leaders earn (at least) the quality advantage, $$\begin{equation}
\mu_{hh}=\mu_{\ell\ell} = \min\left\{\lambda,\frac{\eta}{\eta-1}\right\},
\end{equation}$$ high types leading low types earn both the quality and productivity advantage, $$\begin{equation}
\mu_{h\ell} = \min\left\{\lambda\frac{a_h}{a_\ell},\frac{\eta}{\eta-1}\right\},
\end{equation}$$ and low types leading high types earn $$\begin{equation}
\mu_{\ell h} = \min\left\{\lambda\frac{a_\ell}{a_h},\frac{\eta}{\eta-1}\right\},
\end{equation}$$ We have assumed from the outset that $\lambda>a_h/a_\ell$, so $\mu_{\ell h}>1$ and this market structure is indeed consistent with firm profit incentives. If instead we had $\lambda<a_h/a_\ell$, then this market structure would not exist and we would be left with only the other five.[^75]

### Aggregation 

Substituting the demand curve, $p_{jt}=(Y_{jt}/Y_t)^{-1/\eta}$, and fact that only one firm produces each distinct product, $Y_{jt}=q_{ijt}Y_{ijt}$, into the definition of the markup ([\[eq_app:markup_def\]](#eq_app:markup_def)), we have $$\begin{equation}
w_{{\rm p}t} = \left(\frac{L_{ijt}}{Y_t}\right)^{-1/\eta}(a_iq_{ijt})^{1-1/\eta}\mu_{ijt}^{-1}.
\label{eq_app:prodfoc_1}
\end{equation}$$ Because there is only one firm producing each variety $j$, we can replace $a_i$ with $a_{jt}$ and drop all other $i$ indices (e.g., $q_{ijt}=q_{jt}$). Before aggregating this expression, let us define some useful macroeconomic aggregates. Aggregate quality is defined as an average of blueprint qualities: $$\begin{equation}
Q_t \equiv \left(\frac{1}{N_t}\int_0^{N_t}q_{jt}^{\eta-1}dj\right)^{1/(\eta-1)}.% \\
\end{equation}$$ Aggregate productivity is defined as[^76] $$\begin{equation}
\bar{a}_t \equiv \left(\frac{1}{N_t}\int_0^{N_t}\left(\frac{q_{jt}}{Q_t}\right)^{\eta-1}a_{jt}^{\eta-1}dj\right)^{1/(\eta-1)},
\end{equation}$$ a quality-weighted average of firm-level productivity. Finally, we define two aggregates of markups, $$\begin{equation}
...
\label{eq_app:Mcal_2}
\end{equation}$$ These are averages of markups across products, with higher weight placed on blueprints with higher productivity and quality.

With these objects defined, we can use the expression ([\[eq_app:prodfoc_1\]](#eq_app:prodfoc_1)) to derive two aggregate equations. First, integrating ([\[eq_app:prodfoc_1\]](#eq_app:prodfoc_1)) over $j$ and imposing market clearing $L_{{\rm p}t}=\int_0^{N_t}L_{jt}dj$, we get $$\begin{equation}
w_{{\rm p}t} = \left(\frac{L_{{\rm p}t}}{N_tY_t}\right)^{-1/\eta}(\bar{a}_tQ_t)^{1-1/\eta}\mathcal{M}_{\eta,t}^{-1}.
\label{eq_app:prodfoc_agg_1}
\end{equation}$$ Second, solving for $Y_{jt}$ in ([\[eq_app:prodfoc_1\]](#eq_app:prodfoc_1)) and substituting it into the definition of total output $Y_t$ ([\[eq:cesdemand\]](#eq:cesdemand)), we get $$\begin{equation}
w_{{\rm p}t} =  N_t^{1/(\eta-1)}\bar{a}_tQ_t\mathcal{M}_{\eta-1,t}^{-1}.
\label{eq_app:prodfoc_agg_2}
\end{equation}$$ Now equating the right-hand sides of these expressions and solving for output, we have $$\begin{equation}
Y_t =  N_t^{1/(\eta-1)}\bar{a}_tQ_t\Omega_tL_{{\rm p}t},
\label{eq_app:aggoutput}
\end{equation}$$ where the wedge term $\Omega_t$ is defined $$\begin{equation}
\Omega_t \equiv \frac{\mathcal{M}_{\eta t}^\eta}{\mathcal{M}_{\eta-1,t}^\eta}.
\label{eq_app:omega}
\end{equation}$$ As shown in Section 11.1.5 below, this wedge takes a value in $(0,1]$ and attains its maximum ($\Omega=1$) if and only if there is no cross-product markup heterogeneity ($\mu_{jt}=\mu_t$, $\forall j$). This is because heterogeneous market power distorts demand and results in a misallocation of production labor across blueprints, as in . I therefore refer to $\Omega$ as *static misallocation*.

To solve for aggregate wages and the labor share of output, note that combining the expression ([\[eq_app:aggoutput\]](#eq_app:aggoutput)) with either condition ([\[eq_app:prodfoc_agg_1\]](#eq_app:prodfoc_agg_1)) or condition ([\[eq_app:prodfoc_agg_2\]](#eq_app:prodfoc_agg_2)) implies $$\begin{equation}
w_{{\rm p}t} = \Lambda_t\frac{Y_t}{L_{{\rm p}t}},
\label{eq_app:agglabexp}
\end{equation}$$ where the (production) labor share of output $\Lambda_t = w_{{\rm p}t}L_{{\rm p}t}/Y_t$ has solution $$\begin{equation}
\Lambda_t \equiv \frac{\mathcal{M}_{\eta-1, t}^{\eta-1}}{\mathcal{M}_{\eta t}^\eta}.
\end{equation}$$ Notice that, from the household demand curve, the sales share of good $j$ in total sales is $$\begin{equation}
\frac{p_{jt}Y_{jt}}{Y_t} = p_{jt}^{1-\eta} = \left(\frac{\mu_{jt}w_{{\rm p}t}}{a_iq_{jt}}\right)^{1-\eta},
\end{equation}$$ implying that $$\begin{equation}
\Lambda_t = \int_0^{N_t}\frac{p_{jt}Y_{jt}}{Y_t}\mu_{jt}^{-1}dj
\end{equation}$$ The aggregate labor share is a sales-weighted average of product-level labor shares (inverse markups $\mu_{jt}^{-1}$). A natural definition of the aggregate markup is therefore $\Lambda_t^{-1}$.

### Firm labor allocations, revenues, and profits 

Let us bring back the firm-product-level indices $\{i,j\}$. The conditions ([\[eq_app:prodfoc_1\]](#eq_app:prodfoc_1)) and ([\[eq_app:prodfoc_agg_1\]](#eq_app:prodfoc_agg_1)) together imply labor allocations $$\begin{equation}
...
\label{eq_app:laboralloc}
\end{equation}$$ A product receives a higher labor allocation if it has higher quality, its producer is more productive, or its markup is lower.[^77] Now using the fact that revenue $p_{ijt}Y_{ijt} = \mu_{ijt}w_{{\rm p}t}L_{ijt}$ and the wage $w_{{\rm p}t}=\Lambda_tY_t/L_{{\rm p}t}$, it follows that the firm receives revenues $$\begin{align}
...
\end{align}$$ Fixing relative allocations, the scale of production grows with aggregate output per product $Y$. The corresponding profits are then $$\begin{equation}
\Pi_{ijt} = \left(1-\frac{1}{\mu_{ijt}}\right)p_{ijt}Y_{ijt}.
\label{eq_app:firmprofits}
\end{equation}$$

### Quantification of static misallocation 

Before proceeding, we must verify the claim that the wedge term $\Omega_t$, defined in ([\[eq_app:omega\]](#eq_app:omega)), is indeed in $(0,1]$ and equals one if and only if the labor allocations $L_{ijt}/L_{{\rm p}t}$ maximize aggregate TFP (output per worker $A_t=Y_t/L_{{\rm p}t}$). The maximum level of TFP can be written $$\begin{equation}
...
\end{equation}$$ where $\hat{L}_{ijt}\equiv L_{ijt}/L_{{\rm p}t}$ denotes the share of production labor. Within variety $j$, it is clearly the case that all labor should be allocated to the blueprint with the highest value of $a_iq_{ijt}$, just as occurs the decentralized equilibrium. Letting $\{a_{jt},q_{jt}\}$ denote the values for this blueprint, this means that $$\begin{equation}
A_t^* = \max_{\{\hat{L}_{jt}\}}\left(\int_0^{N_t}(a_{jt}q_{jt}\hat{L}_{jt})^{1-1/\eta}dj\right)^{1/(1-1/\eta)},
\end{equation}$$ subject to the constraint that $$\begin{equation}
1 \geq \int_0^{N_t}\hat{L}_{jt}dj.
\end{equation}$$ Letting $\lambda_{Lt}$ denote the Lagrange multiplier on this constraint, the first-order condition for this maximization is $$\begin{equation}
Y_t^{1/\eta}(q_{jt}a_{jt})^{1-1/\eta}(\hat{L}_{jt}^*)^{-1/\eta} = \lambda_{Lt}.
\end{equation}$$ Aggregating this in the same was as we did the firms' first-order conditions, we have $$\begin{equation}
Y_t^{1/\eta}N_t^{1/\eta}(\bar{a}_tQ_t)^{1-1/\eta} = \lambda_{Lt}.
\end{equation}$$ where $\bar{a}_t$ and $Q_t$ are identical to their values in the decentralized equilibrium. Dividing these two conditions implies $$\begin{equation}
\hat{L}_{jt}^* = \frac{1}{N_t}\left(\frac{a_{jt}q_{jt}}{\bar{a}_tQ_t}\right)^{\eta-1}.
\label{eq_app:laboralloc_efficient}
\end{equation}$$ Substituting this solution back into the demand aggregator, we get $$\begin{equation}
A_t^* = N_t^{1/(\eta-1)}Q_t\bar{a}_t.
\end{equation}$$ Because $\{N_t,Q_t,\bar{a}_t\}$ are the same as in the decentralized equilibrium, the ratio of actual (decentralized) aggregate TFP to this maximal TFP is therefore $$\begin{equation}
\frac{A_t}{A_t^*} = \Omega_t.
\end{equation}$$ We will necessarily have $\Omega_t\in(0,1]$, because $A_t^*\geq A_t>0$ by definition.

To understand what $\Omega_t$ represents, note that $\Omega_t=1$ if and only if there is zero markup dispersion ($\mu_{jt}=\mu_t$, $\forall j$). This can be seen by noticing that the ratio of the actual labor allocation ([\[eq_app:laboralloc\]](#eq_app:laboralloc)) to the maximizing allocation ([\[eq_app:laboralloc_efficient\]](#eq_app:laboralloc_efficient)) equals $$\begin{equation}
\frac{\hat{L}_{jt}}{\hat{L}_{jt}^*} = \left(\frac{\mu_{jt}}{\mathcal{M}_{\eta t}}\right)^{-\eta}.
\end{equation}$$ This ratio equals one for all $j$ if and only if $\mu_{jt}=\mathcal{M}_{\eta t}$ for all $j$ --- that is, if and only if all products earn the same markup.
...
Firm $i$ maximizes the present value $V_{it}$ of its cash flows $D_{it}$, as defined in ([\[eq:firmobjective\]](#eq:firmobjective)). Because the profit maximization at the blueprint level is static (see Section 11.1.4), this objective amounts to choosing R&D and M&A policies given the optimal profits ([\[eq_app:firmprofits\]](#eq_app:firmprofits)) of each product.

To solve this problem, it is helpful to first establish some facts about the state-price density $\xi_t$ which prices firms.[^78] The absence of arbitrage opportunities implies that the riskfree rate $r_{ft}$ equals $$\begin{equation}
r_{ft} = -\mathbb{E}_t\left[\frac{d\xi_t}{\xi_t}\right]\frac{1}{dt}.
\end{equation}$$ and the expected return on any firm $i$ equals $$\begin{equation}
r_{it} \equiv \mathbb{E}_t[dR_{it}]\frac{1}{dt} = r_{ft} - \mathbb{E}_t\left[\frac{d[\xi,V_i]_t}{\xi_tV_{it^-}}\right]\frac{1}{dt}.
\label{eq_app:expret_firm}
\end{equation}$$ Because we have assumed that markets are essentially complete, we will ultimately find that (i) $\xi_t$ is unique and given by the marginal utility of a representative household, and (ii) all expected returns $r_{it}$ equal the riskfree rate $r_{ft}$. But these intuitive results must be proven; for now, we work with the more general expected returns $r_{it}$.

We can rescale the firm's problem to de-trend its growth relative to the aggregate. Everything will be rescaled by per-capita aggregate output $y_t=Y_t/L_t$. Denote rescaled variables with hats: $\hat{Z}_t\equiv Z_t/y_t$. The rescaled dividend $\hat{D}_{it}$ equals rescaled profits, $$\begin{equation}
...
\label{eq_app:rescaled_profits}
\end{equation}$$ minus rescaled R&D and M&A expenditures, $$\begin{equation}
\hat{w}_{{\rm x}t}X_{it} = \hat{w}_{{\rm x}t}(X_{{\rm n}it}+X_{{\rm q}it}+X_{{\rm m}it}).
\label{eq_app:rescaled_investment}
\end{equation}$$ The reason for scaling by per-capita output, as opposed to total output, is that, in a balanced-growth equilibrium, the number of firms $M_t$ and blueprints $N_t$ will grow with the population, so the distribution of rescaled firm sizes (e.g., of $\hat{D}_{it}$) will be stationary. The choice to rescale the skilled wage in ([\[eq_app:rescaled_investment\]](#eq_app:rescaled_investment)), instead of the number of laborers $X_{it}$, has the same rationale.

It follows that the rescaled value of the firm equals $$\begin{equation}
...
\end{equation}$$ Let us derive an HJB equation for this value and express it in terms of scaled variables. First, note that the unscaled firm value can be written recursively as $$\begin{equation}
V_{it} = \mathbb{E}_t\left[\int_t^T\frac{\xi_s}{\xi_t}D_{is}ds\right] + \mathbb{E}_t\left[\frac{\xi_T}{\xi_t}V_{iT}\right],
\end{equation}$$ for any $T>t$. Next, multiplying both sides of this expression by $\xi_t$ and adding $\int_{t_0}^t\xi_sD_{is}ds$, where $t_0<t$, implies: $$\begin{equation}
\int_{t_0}^t\xi_sD_{is}ds + \xi_tV_{it} = \mathbb{E}_t\left[\int_{t_0}^T\xi_sD_{is}ds + \xi_TV_{iT}\right].
\end{equation}$$ Hence, the expression on the left-hand size is a martingale, and so it is also specifically a local martingale: $$\begin{equation}
\mathbb{E}_t\left[d\left(\int_{t_0}^t\xi_sD_{is}ds + \xi_tV_{it}\right)\right] = 0.
\end{equation}$$ Evaluating this differential and using the expected return definition ([\[eq_app:expret_firm\]](#eq_app:expret_firm)) above, we have the unscaled HJB $$\begin{equation}
r_{it}V_{it} = D_{it} + \mathbb{E}_t[dV_{it}]\frac{1}{dt}.
\end{equation}$$ In order to rescale this, note that aggregate output growth, $\dot{y}_t/y_t = g_{yt}$, is deterministic, so the covariation $d[y,\hat{V}_i]_t=0$; thus, we have the decomposition $$\begin{equation}
dV_{it} = d(y_t\hat{V}_{it}) = g_{yt}y_t\hat{V}_{it}dt + y_td\hat{V}_{it}.
\end{equation}$$ Substituting in this decomposition and dividing everything by $y_t$, we get the rescaled HJB $$\begin{equation}
(r_{it} - g_{yt})\hat{V}_{it} = \sup_{\{X_{{\rm n}it},X_{{\rm q}it},X_{{\rm m}it}\}}\left\{\hat{D}_{it} + \mathbb{E}_t[d\hat{V}_{it}]\frac{1}{dt}\right\}.
\label{eq_app:hjb_firm_rescaled}
\end{equation}$$ Henceforth, let $\rho_{it}\equiv r_{it} - g_{yt}$, foreseeing that in equilibrium $\rho_{it}$ will equal the constant preference parameter $\rho$.

### Solving the firm problem 

To solve the firm's problem ([\[eq_app:hjb_firm_rescaled\]](#eq_app:hjb_firm_rescaled)), I first note two facts that help to dramatically simplify the problem. First, note that the rescaled profits of the firm are additively separable across products, and each product's profits depends only on (i) deterministic macroeconomic aggregates (i.e., time) and (ii) the set of blueprint-specific characteristic $Z_{ij} = \{i_1,i_2,\hat{q}\}$, where $\hat{q}\equiv \log(q/Q)$ represents *relative quality*. Letting $\mathcal{Z}_{it}\equiv\{Z_{ijt}\}_{j=1}^{n_{it}}$ denote the collection of blueprint characteristics, firm profits can be written $$\begin{equation}
\hat{\Pi}_{it} = \sum_{j=1}^{n_{it}} \hat{\Pi}_{ijt} \equiv \sum_{\{i_1,i_2,\hat{q}\}\in\mathcal{Z}_{it}}\pi_{i_1i_2\hat{q}t}.
\label{eq_app:profits_sep}
\end{equation}$$ The significance of this is that the profits $\pi_{i_1i_2\hat{q}t}$ are independent of each other and of firm characteristics (except $i_1$). The second useful fact, which will be proven below, is that the optimal R&D and M&A policies $\{X_{{\rm n}it},X_{{\rm q}it},X_{{\rm m}it}\}$ will be linear in the number of blueprints $n$ and, given $n$, will depend only on the firm's type $a_i=a_{i_1}$. Hence, we can define the allocations per blueprint as $x\equiv X/n$ and note that the R&D and M&A production functions can be written as linear functions of $n$. For example, for new-variety creation, we will have $x_{{\rm n}it} \equiv X_{{\rm n}it}/n_{it}$ and $$\begin{equation}
...
\end{equation}$$ Define $\{x_{{\rm q}it},x_{{\rm m}it}\}$ and $\{\Phi_{\rm q}(x),\Psi(x)\}$ analogously for quality innovation and M&A search. A consequence of linearity in $n$ is that total expenditures are linear in $n$: $$\begin{equation}
\hat{w}_{{\rm x}t}X_{it} = \hat{w}_{{\rm x}t}x_{it}n_{it}.
\end{equation}$$ Combining these two facts implies that the rescaled dividend can be written $$\begin{equation}
\hat{D}_{it} = \sum_{\{i_1,i_2,\hat{q}\}\in\mathcal{Z}_{it}}(\pi_{i_1i_2\hat{q}t} - \hat{w}_{{\rm x}t}x_{i_1t}).
\label{eq_app:profits_sep}
\end{equation}$$ Like firm profits, the total firm dividend is separable across blueprint characteristics. Thus, a reasonable conjecture is that the value of the firm is also separable across blueprints: $$\begin{equation}
\hat{V}_{it} = \sum_{\{i_1,i_2,\hat{q}\}\in\mathcal{Z}_{it}}v_{i_1i_2\hat{q}t},
\label{eq_app:value_sep_1}
\end{equation}$$ where $v_{i_1i_2\hat{q}t}$ represents the value of a single blueprint with a leader of type $a_{i_1}$, a follower of type $a_{i_2}$, and relative quality $\hat{q}$.

The separability of the value function ([\[eq_app:value_sep_1\]](#eq_app:value_sep_1)) means that, instead of solving a system of HJB equations across firms, we can instead solve a system of HJBs across blueprint types $\{i_1,i_2,\hat{q}\}$. Let us derive this from the firm-level HJBs ([\[eq_app:hjb_firm_rescaled\]](#eq_app:hjb_firm_rescaled)). Growth in the (rescaled) firm value equals the sum of six components: $$\begin{multline}
...
\label{eq_app:hjb_firmlevelgrowth}
...
Substituting ([\[eq_app:hjb_firmlevelgrowth\]](#eq_app:hjb_firmlevelgrowth)) into the firm-level HJB ([\[eq_app:hjb_firm_rescaled\]](#eq_app:hjb_firm_rescaled)) implies the first-order condition for new-variety R&D $$\begin{equation}
\hat{w}_{{\rm x}t}= \varepsilon(1-\alpha)\varphi\left(\frac{X_{{\rm n}it}}{n_{it}}\right)^{\varepsilon-1}\bar{v}_{{\rm n}it}.
\end{equation}$$ Analogous first-order conditions hold for quality R&D and M&A inputs. These conditions imply the optimal policies $$\begin{align}
\label{eq_app:firmpolicy_newvariety}
...
\label{eq_app:firmpolicy_quality}
...
With these results in hand, we can implement and prove the conjecture ([\[eq_app:value_sep_1\]](#eq_app:value_sep_1)). Recall that, conditional on creating a new variety, the blueprint quality is drawn from the distribution $\Gamma_{\hat{q}}$. Thus, under ([\[eq_app:value_sep_1\]](#eq_app:value_sep_1)), the expected value of a new variety is $$\begin{equation}
\bar{v}_{{\rm n}it} = \int_{-\infty}^\infty v_{i\varnothing\hat{q}t}\Gamma_{\hat{q}}d\hat{q}.
\label{eq_app:expvalue_newvariety}
\end{equation}$$ Recall also that quality innovations are undirected (i.e., target blueprints are chosen at random). This means that the expected present value a new blueprint through quality innovation is $$\begin{equation}
...
\label{eq_app:expvalue_quality}
\end{equation}$$ where $f_{i\hat{q}t}\equiv \sum_{i_2}f_{ii_2\hat{q}t}$. In words, the firm finds a blueprint with current relative quality $\hat{q}$ and creates a new one with relative quality $\hat{q}+\log\lambda$. Finally, to solve for the expected value of M&A, we need to pin down when a deal will take place and, if it does, the deal price. If a firm with type $i$ purchases a blueprint of type $\{i_1=i',i_2,\hat{q}\}$, then the Nash bargaining solution (should it exist) is $$\begin{equation}
...
The blueprint-loss intensities $\delta_{ijt}=\delta_{i_1i_2\hat{q}t}$ in ([\[eq_app:hjb_firmlevelgrowth\]](#eq_app:hjb_firmlevelgrowth)) are pinned down by the fact that the flows of blueprints out of existing markets via quality innovation (creative destruction) must equal flows in: $$\begin{equation}
...
\end{equation}$$ where the policies $x_{{\rm Eq}it}$ are the amount of labor each type-$a_i$ entrepreneur dedicates to quality innovation. In the next section, we will prove that these policies are identical to the incumbent firm policies $x_{{\rm q}it}$. Both terms on the right-hand side are multiplied by the densities $f_{i_1i_2\hat{q}t}=N_{i_1i_2\hat{q}t}/N_t$ because entry is random across blueprints and therefore proportional to the number of existing blueprints with those characteristics. It follows that the solution is $$\begin{equation}
\delta_{i_1i_2\hat{q}t} = \delta_t = \sum_{i\in\{h,\ell\}}\left(f_{it} + \omega_i\frac{\bar{s}L_t}{N_t}\right)\Phi_{{\rm q}i}(x_{{\rm q}it}),
\end{equation}$$ which is independent of blueprint characteristics. Only incumbents can do M&A, and only high-productivity incumbents will choose to do so, so similar logic implies the intensity of M&A exit satisfies $$\begin{equation}
\delta_{i_1i_2\hat{q}t}^\psi N_{i_1i_2\hat{q}t} = \mathbb{I}_{i_1=\ell}\Psi(x_{{\rm m}ht})N_{ht}f_{i_1i_2\hat{q}t} ,
\end{equation}$$ yielding the solution $$\begin{equation}
\delta_{i_1i_2\hat{q}t}^\psi = \delta_{i_1t}^\psi = \mathbb{I}_{i_1=\ell}\Psi(x_{{\rm m}ht})f_h,
\end{equation}$$ which depends on the producer's type (only low types will choose to sell blueprints), but is invariant to the other blueprint characteristics $i_2$ and $\hat{q}$.

Using these intensities along with the values ([\[eq_app:expvalue_newvariety\]](#eq_app:expvalue_newvariety)) through ([\[eq_app:expvalue_ma\]](#eq_app:expvalue_ma)), the HJB ([\[eq_app:hjb_firm_rescaled\]](#eq_app:hjb_firm_rescaled)) with ([\[eq_app:hjb_firmlevelgrowth\]](#eq_app:hjb_firmlevelgrowth)) can be separated into a system of $n_{it}$ HJB equations (with $i=i_1$) of the form $$\begin{multline}
...
\label{eq_app:hjb_blueprintlevelgrowth}
\end{multline}$$ Fixing time $t$, the whole system of HJBs for blueprint values therefore constitutes an exactly identified system across $\{i_1,i_2,\hat{q}\}\in\{h,\ell\}\times\{h,\ell,\varnothing\}\times\mathbb{R}$. This cross-section is still infinite-dimensional because the quality distribution spans $\mathbb{R}$. However, we can reduce it to a finite-dimensional system by using the fact that the profit coefficients $\pi_{i_1i_2\hat{q}t}$ are multiplicatively separable across market structure and quality: $$\begin{equation}
\pi_{i_1i_2\hat{q}t} = \pi_{i_1i_2t}e^{(\eta-1)\hat{q}}.
\label{eq_app:profits_sep_mktvsqual}
\end{equation}$$ Thus, a reasonable conjecture is that the value of a blueprint takes the separable form $$\begin{equation}
v_{i_1i_2\hat{q}t} = v_{i_1i_2t}^{\rm P}e^{(\eta-1)\hat{q}} + v_{i_1t}^{\rm G},
\label{eq_app:value_sep_mktvsqual}
\end{equation}$$ where $v_{i_1i_2t}^{\rm P}e^{(\eta-1)\hat{q}}$ is naturally interpreted as the present value of the blueprint's future profits and $v_{i_1t}^{\rm G}$ the present value of growth opportunities. Substituting ([\[eq_app:value_sep_mktvsqual\]](#eq_app:value_sep_mktvsqual)) into ([\[eq_app:hjb_blueprintlevelgrowth\]](#eq_app:hjb_blueprintlevelgrowth)) and collecting terms in $e^{(\eta-1)\hat{q}}$ implies, for each pair $\{i_1,i_2\}$, the two sets of equations $$\begin{equation}
...
\label{eq_app:hjb_productvalue}
\end{equation}$$ and, using the first-order conditions for R&D and M&A expenditures, $$\begin{multline}
...
\hspace{0.5cm}&& \bar{v}_{{\rm n}it}^{\rm P}\equiv v_{i\varnothing t}^{\rm P}\int_{-\infty}^\infty e^{(\eta-1)\hat{q}}\Gamma_{\hat{q}}d\hat{q}, \\
\bar{v}_{{\rm q}it} &= \bar{v}_{{\rm q}it}^{\rm P} + v_{it}^{\rm G}, 
\hspace{0.5cm}&& \bar{v}_{{\rm q}it}^{\rm P}\equiv \sum_{i'\in\{h,\ell\}}v_{ii't}^{\rm P}\lambda^{\eta-1}\int_{-\infty}^\infty e^{(\eta-1)\hat{q}}f_{i'\hat{q}t}d\hat{q}, \\
\bar{v}_{{\rm m}it} &= \mathbb{I}_{i=h}\varrho f_{\ell t}(\bar{v}_{{\rm m}ht}^{\rm P}-\bar{v}_{{\rm m}\ell t}^{\rm P} + v_{ht}^{\rm G}-v_{\ell t}^{\rm G}), 
\hspace{0.5cm}&& \bar{v}_{{\rm m}it}^{\rm P}\equiv \sum_{i'\in\{h,\ell,\varnothing\}}v_{ii't}^{\rm P}\int_{-\infty}^\infty e^{(\eta-1)\hat{q}}\frac{f_{\ell i'\hat{q}t}}{f_{\ell t}}d\hat{q}.
...
We can combine firms' policies with the expression for aggregate output ([\[eq_app:aggoutput\]](#eq_app:aggoutput)) to compute aggregate economic growth and its components. The growth rate of aggregate output per capita equals $$\begin{equation}
g_{yt} = \frac{1}{\eta-1}g_{Nt} + g_{Qt} + g_{\bar{a}t} + g_{\Omega t} + g_{l_pt}.
\end{equation}$$ It will turn out that only the first two terms, the growth rates of the number of varieties ($g_{Nt}=\dot{N}_t/N_t$) and aggregate quality ($g_{Qt}=\dot{Q}_t/Q_t$), are needed to characterize the equilibrium; thus, I defer discussion of the last three terms to Section [\[app:growthacct\]](#app:growthacct) on growth accounting. Growth in new varieties is given by $$\begin{equation}
g_{Nt} = \sum_{i\in\{h,\ell\}}\Phi_{\rm n}(x_{{\rm n}it})\left(f_{it}+\omega_i\bar{s}\frac{L_t}{N_t}\right).
\end{equation}$$ To compute growth in aggregate quality, note that the change in aggregate quality comes from two sources: new varieties, which have initial quality $\lambda Q_t$; and quality innovations, which improve the current quality of each target blueprint by a factor of $\lambda$. By the chain rule, the change in aggregate quality is $$\begin{equation}
...
\end{equation}$$ The second term can be simplified using a law of large numbers: with intensity $\delta_tdt$, an individual blueprint's quality jumps ($d(q_{jt}^{\eta-1})=(\lambda^{\eta-1}-1)q_{jt}^{\eta-1}$), so we have $$\begin{equation}
\frac{1}{N_t}\int_0^{N_t}d\left(q_{jt}^{\eta-1}\right)dj = \delta_tQ_t^{\eta-1}\left(\lambda^{\eta-1} - 1\right)dt.
\end{equation}$$ Putting all of this together, we have the growth rate $$\begin{equation}
...
Even in the presence of yet-nonexistent cohorts in the household's maximization, the derivation of the household's HJB equation is standard. Under $\varpi_t=L_t$, the utility maximand ([\[eq_app:lifetimeutility_family\]](#eq_app:lifetimeutility_family)) can be written recursively as $$\begin{equation}
U_t = \int_t^Te^{-\rho(s-t)}u_sds + e^{-\rho(T-t)}U_T,
\end{equation}$$ for any $T\geq t$ (I suppress the maximization for convenience). Now, multiplying this equation by $e^{-\rho t}$ and adding $\int_{t_0}^te^{-\rho s}u_sds$ with $t_0<t$ to both sides, we get $$\begin{equation}
\int_{t_0}^te^{-\rho s}u_sds + e^{-\rho t}U_t = \int_{t_0}^Te^{-\rho s}u_sds + e^{-\rho T}U_T.
\end{equation}$$ This equation means the expression on the left-hand side is a martingale; therefore, it is also a local martingale, satisfying the differential equation $$\begin{equation}
\frac{d}{dt}\left(\int_{-\infty}^te^{-\rho s}u_sds + e^{-\rho t}U_t\right) = 0.
\end{equation}$$ Evaluating this expression and bringing back the maximization implies the HJB equation $$\begin{equation}
\rho U_t = \sup_{\alpha_t,\{c_{kt},l_{kt},x_{kt}\}}\left\{u_t + \dot{U}_t\right\}.
\label{eq_app:hjb_1}
\end{equation}$$ To solve this, we must express $U_t$ as a function of the state variables of the household.

The first state variable of the household is its total financial wealth $W_t$. The household accumulates wealth through three endogenous sources of income. The first is labor income,[^80] $$\begin{equation}
w_{{\rm p}t}L_{{\rm p}t} + w_{{\rm x}t}L_{{\rm x}t} = \int_0^{L_t}(w_{{\rm p}t}l_{pkt} + w_{{\rm x}t}(l_{xkt}-x_{{\rm En}kt}-x_{{\rm Eq}kt}))dk.
\end{equation}$$ The second source is new-firm creation from entrepreneurs. Let $dJ_{{\rm n}kt}\in\{0,1\}$ denote a Poisson jump that equals one if individual $k$ succeeds in starting a new firm via new-variety creation. The assumption that entrepreneurs have the same R&D technologies as single-product firms means $$\begin{equation}
\mathbb{E}_tdJ_{{\rm n}kt} = \Phi_{\rm n}(x_{{\rm En}kt})dt = (1-\alpha)\varphi x_{{\rm En}kt}^\varepsilon dt.
\end{equation}$$ Define $dJ_{{\rm q}kt}$ analogously for a new firm formed via quality innovation. Let $v_{{\rm En}kt}$ and $v_{{\rm Eq}kt}$ denote the market value of a newly created firm. Then the total flow of new-firm value equals $$\begin{equation}
...
\end{equation}$$ The fact that there is a continuum of entrepreneurs means two things. First, policies will be symmetric across technologies and types (high- versus low-productivity $a_i$), so there are only four unique policies $\{x_{{\rm En}ht},x_{{\rm Eq}ht},x_{{\rm En}\ell t},x_{{\rm Eq}\ell t}\}$. Second, we can apply a law of large numbers to express this value as a deterministic function of policies: $$\begin{equation}
...
\end{equation}$$ where $\bar{v}_{{\rm En}it}$ and $\bar{v}_{{\rm Eq}it}$ are the expected values of a new firm. By the results in the previous section, we will have $\bar{v}_{{\rm En}it} = \bar{v}_{{\rm n}it}y_t$ and $\bar{v}_{{\rm Eq}it}=\bar{v}_{{\rm q}it}y_t$. Finally, the household receives capital income from trading in the stocks of incumbent firms and in the riskfree bond with return $r_{ft}$. Let $\alpha_t$ denote the share of wealth invested in the stock market[^81] and $$\begin{equation}
r_{Vt} \equiv \frac{D_t}{V_t} + \frac{\dot{V}_t-V_{{\rm E}t}}{V_t}
\end{equation}$$ the return on all *incumbent* firms in the economy.[^82] Combining these three sources with consumption expenditures $C_t \equiv \int_0^{L_t}c_{kt}dk$, wealth grows according to $$\begin{equation}
\dot{W}_t = W_t(r_{ft} + \alpha_t(r_{Vt}-r_{ft})) + V_{{\rm E}t} + w_{{\rm p}t}L_{{\rm p}t} + w_{{\rm x}t}L_{{\rm x}t} - C_t.
\end{equation}$$ As a reality check, note that imposing market clearing for wealth $W_t = V_t$ (and $\alpha_t=1$) and consumption $C_t=Y_t$ implies $$\begin{equation}
\dot{W}_t = (D_t + \dot{V}_t - V_{{\rm E}t}) + V_{{\rm E}t} + w_{{\rm p}t}L_{{\rm p}t} + w_{{\rm x}t}L_{{\rm x}t} - C_t = \dot{V}_t + Y_t - C_t = \dot{V}_t,
\end{equation}$$ consistent with the fact that all tradable wealth in the economy is held in claims to (existing) firms' cash flows (i.e., the stock market).

The second state variable of the household captures expected growth in off-balance-sheet wealth, such as future labor income. We will prove that aggregate output $Y_t$ and time $t$ are sufficient information to summarize this state. Consider, as an example, the present value of aggregate income from production labor: $$\begin{equation}
H_{{\rm p}t} \equiv \int_0^\infty e^{-\int_0^\tau r_{f,t+\tau'}d\tau'}w_{p,t+\tau}L_{p,t+\tau}d\tau.
\end{equation}$$ We showed in Section 11.1.3 that total income $w_{{\rm p}t}L_{{\rm p}t}=\Lambda_tY_t$ for labor share $\Lambda_t$ a deterministic function of time. Thus, the present value of production-labor income can be stated as $$\begin{equation}
H_{{\rm p}t} = Y_t \int_0^\infty e^{-\int_0^\tau (r_{f,t+\tau'}-g_{Y,t+\tau'})d\tau'}\Lambda_{t+\tau}d\tau = Y_th_p(t),
\end{equation}$$ the product of aggregate output and a deterministic function of time. Similar arguments can be made for human wealth from skilled labor and new-firm creation. Unlike wealth $W_t$, which has an endogenous accumulation equation, the household takes aggregate output and its growth rate $\dot{Y}_t/Y_t=g_{Yt}$ as given.

To solve the HJB equation ([\[eq_app:hjb_1\]](#eq_app:hjb_1)), I first conjecture that $U_t$ is a function of wealth, aggregate output, and time, implying that $$\begin{equation}
...
\label{eq_app:hjb_2}
\end{equation}$$ Consistent with the absence of systematic risks, the first-order condition for the allocation $\alpha_t$ implies the return restriction $$\begin{equation}
r_{Vt} = r_{ft},
\end{equation}$$ as otherwise there would be an arbitrage opportunity. The first-order conditions for consumption are $$\begin{equation}
\frac{c_{kt}^{-1}}{L_t} = \frac{\partial U_t}{\partial W_t},
\label{eq_app:hhfoc_cons}
\end{equation}$$ which implies that all individuals consume the same amount $c_{kt}=c_t=C_t/L_t$. The first-order conditions for production laborers are $$\begin{equation}
\frac{\chi l_{pkt}^{1/\zeta}}{L_t} = \frac{\partial U_t}{\partial W_t}w_{{\rm p}t},
\label{eq_app:hhfoc_labsup_p}
\end{equation}$$ which, similarly, implies that $l_{pkt}=l_{{\rm p}t}$. Likewise, for skilled laborers, we have $$\begin{equation}
\frac{\chi l_{xkt}^{1/\zeta}}{L_t} = \frac{\partial U_t}{\partial W_t}w_{{\rm x}t},
\label{eq_app:hhfoc_labsup_x}
\end{equation}$$ so $l_{xkt}=l_{{\rm x}t}$. Finally, the first-order conditions for type-$a_i$ entrepreneurial effort into each technology is $$\begin{equation}
...
\label{eq_app:hhfoc_ent_}
\end{equation}$$ Entrepreneurs weigh the expected market value of their investment against the foregone skilled wage they would have received if they had instead worked for an incumbent. Dividing these by $y_t$ implies that the entrepreneurs have exactly the same first-order conditions as incumbents, and therefore the exact same R&D policies conditional on type.

Next, conjecture that the solution to the value function is $$\begin{equation}
U_t = k(t) + \rho^{-1}\log\left(\frac{W_t+Y_th(t)}{L_t}\right)
\label{eq_app:hjb_conject}
\end{equation}$$ for some functions $k(t)$ and $h(t)$ of time, for which we will solve. In line with the intuition above, the function $h(t)$ will be such that $Y_th(t)$ represents the present value of non-financial wealth. Foreseeing this, let us define total wealth as $$\begin{equation}
\overline{W}_t \equiv W_t+Y_th(t).
\end{equation}$$ It will prove to be the case that $\overline{W}_t/L_t$ equals the present value of per-capita consumption. Now substitute the conjecture ([\[eq_app:hjb_conject\]](#eq_app:hjb_conject)) into the first-order conditions ([\[eq_app:hhfoc_cons\]](#eq_app:hhfoc_cons)) through ([\[eq_app:hhfoc_labsup_x\]](#eq_app:hhfoc_labsup_x)). We then have per capita consumption $$\begin{equation}
c_t = \rho \frac{\overline{W}_t}{L_t},
\label{eq_app:optpolicy_hh_cons}
\end{equation}$$ the standard result for a unit EIS. Combining this with the market-clearing condition $c_t=Y_t/L_t$, the labor supply first-order conditions ([\[eq_app:hhfoc_labsup_p\]](#eq_app:hhfoc_labsup_p)) and ([\[eq_app:hhfoc_labsup_x\]](#eq_app:hhfoc_labsup_x)) imply $$\begin{equation}
...
\end{equation}$$ These labor supply curves can be combined with the labor demand curves from firms to solve for labor hours and wages. For production labor, we have the aggregate demand curve ([\[eq_app:agglabexp\]](#eq_app:agglabexp)) with $L_{{\rm p}t}=(1-\bar{s})l_{{\rm p}t}L_t$, implying the solution $$\begin{equation}
l_{{\rm p}t} = \left(\frac{\Lambda_t}{\chi(1-\bar{s})l_{{\rm p}t}}\right)^\zeta = \left(\frac{\Lambda_t}{\chi(1-\bar{s})}\right)^{\frac{\zeta}{1+\zeta}}.
\label{eq_app:optpolicy_hh_lp}
\end{equation}$$ For skilled labor, the policies ([\[eq_app:firmpolicy_newvariety\]](#eq_app:firmpolicy_newvariety)) through ([\[eq_app:firmpolicy_ma\]](#eq_app:firmpolicy_ma)) imply total demand from incumbents $$\begin{equation}
...
\end{equation}$$ where, from ([\[eq_app:firmpolicy_newvariety\]](#eq_app:firmpolicy_newvariety)) to ([\[eq_app:firmpolicy_ma\]](#eq_app:firmpolicy_ma)), the object $$\begin{equation}
\kappa_t^{\rm inc} \equiv \sum_{i\in\{h,\ell\}}f_{it}\hat{w}_{{\rm x}t}^{1/(1-\varepsilon)}(x_{{\rm n}it}+x_{{\rm q}it}+x_{{\rm m}it})
\end{equation}$$ is independent of the wage $\hat{w}_{{\rm x}t}$. Recall that labor hours $l_{{\rm x}t}$ include hours supplied to both incumbents and private entrepreneurial efforts. Thus, the total skilled labor supply that must meet this demand is total hours $L_{{\rm x}t}=\bar{s}l_{{\rm x}t}L_t$ minus entrepreneurial hours $$\begin{equation}
...
\end{equation}$$ where, analogous to $\kappa_t^{\rm inc}$, $$\begin{equation}
\kappa_t^{\rm ent} \equiv \sum_{i\in\{h,\ell\}}\omega_i\hat{w}_{{\rm x}t}^{1/(1-\varepsilon)}(x_{{\rm n}it}+x_{{\rm q}it}).
\end{equation}$$ Now equate aggregate labor supply with demand, $$\begin{equation}
...
\end{equation}$$ and solve for the wage: $$\begin{equation}
...
\label{eq_app:optpolicy_hh_skilledwage}
\end{equation}$$ Substituting this back into the supply curve gives $$\begin{equation}
l_{{\rm x}t} = (\chi^{-1}\hat{w}_{{\rm x}t})^\zeta.
\label{eq_app:optpolicy_hh_lx}
\end{equation}$$

To verify the conjecture, substitute the optimal policies above and the conjecture ([\[eq_app:hjb_conject\]](#eq_app:hjb_conject)) into the HJB ([\[eq_app:hjb_2\]](#eq_app:hjb_2)). First, note that the utility flow becomes $$\begin{equation}
...
\end{equation}$$ where the labor supply terms are functions of time only. Second, note that $W_t=V_t$. Thus, the HJB becomes $$\begin{multline}
...
\label{eq_app:hjb_conject_ode}
\end{multline}$$ Because the ratio $V_t/Y_t$ is a function of time only (proven formally in the next section), so too are the ratios $$\begin{equation}
\frac{V_t}{\overline{W}_t} = \frac{V_t/Y_t}{V_t/Y_t+h(t)} \quad\textrm{and}\quad \frac{Y_th(t)}{\overline{W}_t} = \frac{h(t)}{V_t/Y_t+h(t)}.
\end{equation}$$ This confirms the form of the conjecture ([\[eq_app:hjb_conject\]](#eq_app:hjb_conject)). However, the ordinary differential equation ([\[eq_app:hjb_conject_ode\]](#eq_app:hjb_conject_ode)) only characterizes $k(t)$ in terms of the function $h(t)$. The next section computes $h(t)$ analytically. In all parts of the paper, including the transition path, the specific functional form of $k(t)$ turns out to be irrelevant, because policies and resource allocations do not explicitly depend on $k(t)$. All that matters is that it exists, which ([\[eq_app:hjb_conject_ode\]](#eq_app:hjb_conject_ode)) confirms.

### State-price density and asset prices 

The state-price density of the household equals $$\begin{equation}
\xi_t = e^{-\rho t}\frac{\partial u_t}{\partial c_t} = e^{-\rho t}c_t^{-1},
\end{equation}$$ where, again, per-capita consumption $c_t$ equals per-capita output $y_t$. This implies the law of motion $$\begin{equation}
\frac{\dot{\xi}_t}{\xi_t} = -\rho - g_{yt}.
\end{equation}$$ By the absence of arbitrage, the riskfree rate is equal to $$\begin{equation}
r_{ft} = -\frac{\dot{\xi}_t}{\xi_t} = \rho + g_{yt},
\end{equation}$$ and all risk premia are equal to zero. Hence, the growth-adjusted discount rate applied to each firm, described in Appendix 11.2.1, is $$\begin{equation}
\rho_{it} = r_{ft} - g_{yt} = \rho.
\end{equation}$$

We can use this discount rate to compute $h(t)$ in ([\[eq_app:hjb_conject\]](#eq_app:hjb_conject)). Note that the present value of per-capita consumption equals $$\begin{equation}
\int_0^\infty e^{-\int_0^\tau r_{f,t+\tau'}d\tau'}c_{t+\tau}d\tau = c_t\int_0^\infty e^{-\rho \tau}d\tau = \frac{c_t}{\rho} = \frac{\overline{W}_t}{L_t},
\end{equation}$$ where the last equation follows from the household's optimal consumption policy. Thus, per-capita total wealth $\overline{W}_t/L_t$ is the present value of consumption. Using the definition of total wealth and solving for $h(t)$, we get $$\begin{equation}
h(t) = \frac{1}{\rho} - \frac{V_t}{Y_t},
\label{eq_app:pvnonfinwealth}
\end{equation}$$ where $$\begin{equation}
...
\label{eq_app:valueoutputratio}
\end{equation}$$ which is indeed a function of time only. Substituting ([\[eq_app:pvnonfinwealth\]](#eq_app:pvnonfinwealth)) and ([\[eq_app:valueoutputratio\]](#eq_app:valueoutputratio)) back into ([\[eq_app:hjb_conject_ode\]](#eq_app:hjb_conject_ode)) characterizes the solution for $k(t)$, with terminal condition $\lim_{t\to\infty}k(t)$ equal to the constant solution in the balanced-growth equilibrium.
...
\end{equation}$$ Dividing by $\Delta t$ and taking the limit as the time interval becomes small implies $$\begin{equation}
...
\end{equation}$$ This is the standard KFE for a process with only negative drift.
...
\end{multline}$$ Dividing by $N_t$ and using the fact that $\dot{f}_{Zt} = \dot{N}_{Zt}/N_t-(\dot{N}_t/N_t)f_{Zt}$ implies the KFE $$\begin{equation}
...
\label{eq_app:kfe_monop}
\end{equation}$$ Similarly, for a same-type leader-follower pair $i_1=i_2=i$, and letting $-i\equiv\{h,\ell\}\setminus\{i\}$, we have blueprint flows $$\begin{multline}
...
\label{eq_app:kfe_opptype}
\end{multline}$$ Given R&D and M&A policies, the KFEs ([\[eq_app:kfe_monop\]](#eq_app:kfe_monop)), ([\[eq_app:kfe_sametype\]](#eq_app:kfe_sametype)), and ([\[eq_app:kfe_opptype\]](#eq_app:kfe_opptype)) fully characterize the time path of the blueprint distribution.

We can aggregate these KFEs to characterize the evolution of the total blueprint share across firm types $f_{it}$. Specifically, integrating the KFEs for $i_1=h$ across qualities and summing over follower types implies[^83] $$\begin{equation}
\dot{f}_{ht} + g_{Nt}f_{ht} = \Phi_{{\rm n}ht}(f_{ht} + \nu_{ht}) + \Phi_{{\rm q}ht}(f_{ht} + \nu_{ht}) + \Psi_{ht}f_{ht}f_{\ell t} - \delta_tf_{ht}.
\label{eq_app:kfe_hightypeall}
\end{equation}$$ Substituting in $g_{Nt}$ and $\delta_t$ implies $$\begin{equation}
...
\end{equation}$$ This expression gives some insight into why equilibria are stable and eventually converge to a balanced-growth path in which both types maintain positive market share. If low-productivity firms have all the market share ($f_{\ell t}=1$), then $$\begin{equation}
\dot{f}_{ht} = (\Phi_{{\rm n}ht} + \Phi_{{\rm q}ht})\nu_{ht} > 0.
\end{equation}$$ The flow of new high-productivity entrants increases the market share. Conversely, if high-productivity firms produce all goods ($f_{ht}=1$), then $$\begin{equation}
\dot{f}_{ht} = -(\Phi_{{\rm n}\ell t} + \Phi_{{\rm q}\ell t})\nu_{\ell t} < 0.
\end{equation}$$ In both of these extreme cases, there are no flows from M&A because, in the first case ($f_{\ell t}=1$), there are no buyers looking for targets; and in the second ($f_{ht}=1$), there are no targets to identify.
...
\end{equation}$$ confirming that high-productivity firms earn strictly greater operating profits on a blueprint.

Next, let us prove the claim that $v_{hi_2\hat{q}}^{\rm P}>v_{\ell i_2\hat{q}}^{\rm P}$ and $v_h^{\rm G}>v_\ell^{\rm G}$. The proof is clearest if we first consider the case without M&A ($\psi=0$). In this case, the ratio of product values is $$\begin{equation}
\frac{v_{hi_2\hat{q}}^{\rm P}}{v_{\ell i_2\hat{q}}^{\rm P}} = \frac{\pi_{hi_2\hat{q}}}{\pi_{\ell i_2\hat{q}}} > 1.
\end{equation}$$ This implies that the ratio of growth values is $$\begin{equation}
...
\end{equation}$$ This implies that $v_{hi_2\hat{q}}^{\rm P}>v_{\ell i_2\hat{q}}^{\rm P}$ for all blueprint types, confirming the claim.
...
\end{equation}$$ where $g_{Mt}\equiv\dot{M}_t/M_t$ is the net growth rate of the number of firms.
...
\label{eq_app:firmsize_cond_kfe}
\end{multline}$$ The solution to this differential-difference equation is the function $$\begin{equation}
...
\label{eq_app:firmsize_cond_solution}
\end{equation}$$ This solution can be derived via the probability-generating function of , as laid out in and . Alternatively, one could verify this solution by simply substituting ([\[eq_app:firmsize_cond_solution\]](#eq_app:firmsize_cond_solution)) into the conditional KFEs ([\[eq_app:firmsize_cond_kfe\]](#eq_app:firmsize_cond_kfe)).

### Distribution of age and type 

Aggregating the KFEs for $m_i(n,\tau)$ over $n$ for any $\tau>0$, we have $$\begin{equation}
g_Nm_i(\tau) = -\frac{\partial m_i(\tau)}{\partial \tau} - \bar{\delta}_i m_i(1,\tau).
\label{eq_app:firmdist_age}
\end{equation}$$ Dividing by $m_i(\tau)$ and integrating from over $\tau$ implies that the solution to this is $$\begin{equation}
m_i(\tau) = \kappa_i\exp\left\{-\left(g_N\tau + \bar{\delta}_i\int_0^\tau m(1|a_i,\tau')d\tau'\right)\right\},
\label{eq_app:firmsize_agetype}
\end{equation}$$ for some constants of integration $\kappa_i$. Using the conditional size distribution ([\[eq_app:firmsize_cond_solution\]](#eq_app:firmsize_cond_solution)) at $n=1$, $$\begin{equation}
m(1|\tau,a_i) = \frac{(\bar{\Phi}_i-\bar{\delta}_i)e^{-(\bar{\Phi}_i-\bar{\delta}_i)\tau}}{\bar{\Phi}_i-\bar{\delta}_ie^{-(\bar{\Phi}_i-\bar{\delta}_i)\tau}},
\end{equation}$$ the integral in ([\[eq_app:firmsize_agetype\]](#eq_app:firmsize_agetype)) evaluates to $$\begin{equation}
...
\end{equation}$$ If $\bar{\Phi}_i>\bar{\delta}_i$ (type-$a_i$ firms grow on average), then this grows asymptotically to a constant $\log(\bar{\Phi}_i/(\bar{\Phi}_i-\bar{\delta}_i))$. If $\bar{\Phi}_i<\bar{\delta}_i$, then this can be written as $$\begin{equation}
...
\end{equation}$$ which asymptotically grows linearly with $\tau$.

The last step in solving for $m_i(\tau)$ is finding the constants $\kappa_i$ in ([\[eq_app:firmsize_agetype\]](#eq_app:firmsize_agetype)). These expressions need to integrate to the total firm-type shares $m_i=\int_0^\infty m_i(\tau)d\tau$, so it must be that $$\begin{equation}
...
\end{equation}$$ To solve for $m_i$, and thus for $m_i(\tau)$, note first that, at $\tau=0$, we have $$\begin{equation}
m_i(\tau=0) = \kappa_i = \frac{m_i}{\bar{m}_i},
\label{eq_app:firmsize_mi0_kappa}
\end{equation}$$ and hence the conditional share of type-$a_i$ firms at $\tau=0$ is $$\begin{equation}
m(a_i|\tau=0) = \frac{m_i(\tau=0)}{m_h(\tau=0)+m_\ell(\tau=0)} = \frac{m_i/\bar{m}_i}{m_h/\bar{m}_h+m_\ell/\bar{m}_\ell}.
\end{equation}$$ Second, note that the conditional share at $\tau=0$ is also equal to the relative rates of entry: $$\begin{equation}
m(a_i|\tau=0) = \frac{\omega_i(\Phi_{{\rm n}i}+\Phi_{{\rm q}i})}{\omega_h(\Phi_{{\rm n}h}+\Phi_{{\rm q}h})+\omega_\ell(\Phi_{{\rm n}\ell}+\Phi_{{\rm q}\ell})}.
\end{equation}$$ Equating these two expressions and solving for $m_i$ implies $$\begin{equation}
...
\end{equation}$$ Substituting this back into ([\[eq_app:firmsize_mi0_kappa\]](#eq_app:firmsize_mi0_kappa)), we have $$\begin{equation}
...
\end{equation}$$ and hence a closed-form solution for $m_i(\tau)$.

Finally by Bayes' rule, we can multiply the conditional distribution ([\[eq_app:firmsize_cond_solution\]](#eq_app:firmsize_cond_solution)) with the marginal distribution ([\[eq_app:firmsize_agetype\]](#eq_app:firmsize_agetype)) to get the solution $m_i(n,\tau)$.

## Derivation of firm entry and exit rates 

The firm entry rate $\mathcal{E}_{\rm entry}$ is defined as the flow of new firms being created relative to the number of incumbent firms: $$\begin{equation}
...
\label{eq_app:entrate_t}
\end{equation}$$ The ratio $N_t/M_t$ can be computed from the marginal distribution $m_t(n)=\sum_i\int_0^\infty m_{it}(n,\tau)d\tau$: $$\begin{equation}
\frac{N_t}{M_t} = \sum_{n=1}^\infty nm_t(n).
\end{equation}$$ The exit rate is likewise defined as the flow of firms existing relative to the number of incumbent firms: $$\begin{equation}
\mathcal{E}_{{\rm exit},t} = \frac{\sum_{i\in\{h,\ell\}}\bar{\delta}_{it}M_{it}(n=1)}{M_t} = \sum_{i\in\{h,\ell\}}\bar{\delta}_{it}m_{it}(n=1).
\end{equation}$$ In a balanced-growth equilibrium, the entry and exit rates are constant and are linked to each other by the relation $$\begin{equation}
g_L = \mathcal{E}_{\rm entry} - \mathcal{E}_{\rm exit},
\end{equation}$$ because the growth rate in the number of firms $g_M = \dot{M}_t/M_t$ is equal to the growth rate of the population $g_L$.

## Derivation of Pareto tail coefficients 

As shows, if $\bar{\Phi}_i>\bar{\delta}_i$, then the distribution of blueprints $m_i(n)$ exhibits a Pareto tail with tail index $\theta_i>0$. To represent the fact that the probability density function $m_i(n)$ behaves like $n^{-(1+\theta_i)}$, I use the notation $m_i(n)\stackrel{\rm lim}{\sim}n^{-(1+\theta_i)}$.[^84] To show this, it is easiest to first show that the firm-product distribution $p_i(n)\propto nm_i(n)\stackrel{\rm lim}{\sim}n^{-\theta_i}$. The tail index is defined as the limit of the sequence $$\begin{equation}
\theta_i \equiv \lim_{n\to\infty} - n \frac{p_i(n+1) - p_i(n)}{p_i(n)}.
\end{equation}$$ Intuitively, this tail index behaves like the continuous elasticity $\lim_{n\to\infty}-\partial\log p_i(n)/\partial\log n$ and is finite if $\log p_i(n)$ falls at the same rate as (or more slowly than) $\log n$ rises. If the distribution exhibits a tail that is thinner than a Pareto distribution (which will occur for $\bar{\Phi}_i\leq\bar{\delta}_i$), then the tail index will be infinite.

Consider the case $\bar{\Phi}_i>\bar{\delta}_i$. To compute the Pareto tail coefficient, note that the KFE for $m_i(n)$ is $$\begin{equation}
g_Nm_i(n) = \bar{\Phi}_i[(n-1)m_i(n-1) - nm_i(n)] + \bar{\delta}_i[(n+1)m_i(n+1) - nm_i(n)].
\end{equation}$$ Dividing both sides by $N_t$, we can rewrite this in terms of the firm-product distribution $$\begin{equation}
g_N\frac{1}{n}p_i(n) = \bar{\Phi}_i[p_i(n-1) - p_i(n)] + \bar{\delta}_i[p_i(n+1) - p_i(n)].
\label{eq_app:firmsize_margdist_size}
\end{equation}$$ Multiplying both sides by $n/p_i(n)$ and taking limits as $n\to\infty$, we then get the tail coefficient $$\begin{equation}
\theta_i = \frac{g_N}{\bar{\Phi}_i-\bar{\delta}_i}.
\end{equation}$$ If instead $\bar{\Phi}_i<\bar{\delta}_i$, then the tail is thinner than Pareto. We instead have $p_i(n)\stackrel{\rm lim}{\sim}(\bar{\Phi}_i/\bar{\delta}_i)^n$. One can verify this by dividing ([\[eq_app:firmsize_margdist_size\]](#eq_app:firmsize_margdist_size)) by $p_i(n)$, substituting this in, and taking $n\to\infty$. As shown in , the tail index for the entire distribution of firms (i.e., including both high and low types) is $$\begin{equation}
\theta = \min\{\theta_h,\theta_\ell\} = \min\left\{\frac{g_N}{[\bar{\Phi}_h-\bar{\delta}_h]^+},\frac{g_N}{[\bar{\Phi}_\ell-\bar{\delta}_\ell]^+}\right\} = \theta_h.
\end{equation}$$ High-productivity firms grow faster than low-productivity firms on average, so they dominate the tail.

As a final step, let us prove that the tail index $\theta$ of the blueprint distribution also applies to the distribution of employment and the distributions of rescaled firm revenues, profits, and value. Firm $i$'s total employment per blueprint equals $$\begin{equation}
...
The markups of a product $\mu_{i_1i_2}$ are independent of product values and the distribution, so these can be computed with initialized values. The profit rate at iteration $k$ then equals $$\begin{equation}
...
\end{equation}$$ where $\bar{a}^{k-1}$ and $\mathcal{M}_{\eta-1}^{k-1}$ are computed using $\{f_{i_1i_2\hat{q}}^{k-1}\}$.

Using the distribution $\{f_{i_1i_2\hat{q}}^{k-1}\}$, the policies $\{x_{{\rm n}i}^{k-1},x_{{\rm q}i}^{k-1},x_{{\rm m}i}^{k-1}\}$, and the ratio $LN^{k-1}$, we can compute product values as $$\begin{equation}
...
\end{equation}$$ Using these product values, we can compute expected product values, $\{\bar{v}_{{\rm n}i}^{k},\bar{v}_{{\rm q}i}^{k},\bar{v}_{{\rm m}i}^{k}\}$, with which we can compute the value of growth opportunities. Because of M&A, growth values are interdependent in the HJB, so we need to invert a two-by-two system: for $v^{\rm G} = [v_h^{\rm G}\quad v_\ell^{\rm G}]^\top$, $$\begin{equation}
C^{{\rm G},k}v^{{\rm G},k} = \tilde{v}^{{\rm G},k},
\end{equation}$$ where (suppressing $k$ superscripts for the moment) $$\begin{equation}
...
\end{bmatrix}.
\end{equation}$$ Inverting this system gives us $\{v_{i_1}^{{\rm G},k}\}$.
...
Within iteration $k$, we re-initialize the procedure at iteration $l=0$, setting $\{LN^0,\hat{w}_{\rm x}^0,f_h^0\}$ equal to their values $\{LN^{k-1},\hat{w}_{\rm x}^{k-1},f_h^{k-1}\}$ from the previous outer-loop iteration. We can then calculate the new-variety R&D policies $$\begin{equation}
x_{{\rm n}i}^l = \left(\frac{\varepsilon(1-\alpha)\varphi}{\hat{w}_{\rm x}^{l-1}}(\bar{v}_{{\rm n}i}^{{\rm P},k} + v_i^{{\rm G},k})\right)^{1/(1-\varepsilon)},
\end{equation}$$ and do the same for $x_{{\rm q}i}^{k}$ and $x_{{\rm m}i}^{k}$. These imply $\{\Phi_{{\rm n}i}^l,\Phi_{{\rm q}i}^l,\Psi_i^l\}$. Next, compute the $LN^l$ that solves $g_N=g_L$, or $$\begin{equation}
LN^l = \frac{g_L - \sum_{i\in\{\ell,h\}}f_i^l\Phi_{{\rm n}i}^l}{\bar{s}\sum_{i\in\{\ell,h\}}\omega_i\Phi_{{\rm n}i}^l}
\end{equation}$$ Then find $f_h^l$ that solves $$\begin{equation}
(g_L + \Phi_{{\rm q}h}^l(\nu_h^l+f_h) + \Phi_{{\rm q}\ell}^l(\nu_\ell^l+f_\ell))f_h^l = (\Phi_{{\rm n}h}^l+\Phi_{{\rm q}h}^l)(\nu_h^l + f_h^l) + \Psi_h^lf_h^l(1-f_h^l).
\end{equation}$$ Substituting $f_\ell^l=1-f_h^l$, this is a quadratic equation, $$\begin{multline}
...
\end{align}$$ Finally, we can re-solve for the skilled wage $$\begin{equation}
...
\end{equation}$$ The criterion for the convergence of this inner loop is that $LN$ is constant between iterations: $|LN^l-LN^{l-1}|<\epsilon_{\rm inner}^{\rm tol}$, where I use the threshold $\epsilon_{\rm inner}^{\rm tol}=10^{-8}$. At this final iteration $l$, we take these values for $\{x_{{\rm n}i}^k,x_{{\rm q}i}^k,x_{{\rm m}i}^k\}$, $\{\Phi_{{\rm n}i}^k,\Phi_{{\rm q}i}^k,\Psi_i^k\}$, $LN^k$, $\hat{w}_{\rm x}^k$, and $f_h^k$.

### Solving KFEs

Given $f_h$, we can solve the KFEs as a $(6\times(2K+1))$-dimensional linear system. For notational simplicity, suppress the iteration index $k$ in what follows. Note that the derivative over relative quality $\hat{q}$ can be approximately by the discretization $$\begin{equation}
\frac{\partial f_{i_1i_2\hat{q}}}{\partial\hat{q}} = \frac{f_{i_1i_2,\hat{q}+\log\lambda/m}-f_{i_1i_2\hat{q}}}{\log\lambda/m}
\end{equation}$$ We take a forward difference because $\hat{q}$ is falling over time.

We will use the following notation. Let $C_{ss'}$ denote a $(2K+1)\times(2K+1)$ matrix of the form $$\begin{equation}
C_{ss'} = \begin{bmatrix}
...
\end{bmatrix},
\end{equation}$$ for constants $c_{ss'}^{(j)}$ to be defined. The indices $s$ correspond to the states $\{i_1,i_2\}$ ordered from 1 to 6 as $\{\{h,\varnothing\},\{\ell,\varnothing\},\{h,h\},\{\ell,\ell\},\{h,\ell\},\{\ell,h\}\}$; the same is true of $s'$. The interpretation of element $c_{ss'}^{(j)}$ is thus the transition rate into the current state $s$ and quality $\hat{q}$ (the row) from the state $s'$ with quality $j$ increments higher than $\hat{q}$.

Now, let us concatenate these block matrices into the $6\times6$ block matrix $$\begin{equation}
C = \begin{bmatrix}
...
\end{bmatrix},
\end{equation}$$ which ultimately is a $6(2K+1))\times6(2K+1))$ matrix.

Letting $\mathbf{f}$ denote the $(6(2K+1))$-dimensional vector of the distribution $f_{i_1i_2\hat{q}}$ with element order corresponding to the order in $C$, we can write the KFEs as the linear system $$\begin{equation}
C\mathbf{f} = \mathbf{v},
\end{equation}$$ where the right-hand side vector $\mathbf{v}$ is a concatenation of the new-variety quality distribution and a vector of zeros: $$\begin{equation}
\mathbf{v} \equiv [\Phi_{{\rm n}h}(\nu_h+f_h)\Gamma^\top,\Phi_{{\rm n}\ell}(\nu_\ell+f_\ell)\Gamma^\top,0_{4(2K+1)}^\top]^\top.
\end{equation}$$ (Recall that we have assumed $\Gamma_{\hat{q}}$ is a point mass at $\hat{q}=\log\lambda$.) From this equation, it follows that the diagonal block matrices $C_{ss}$ have elements $$\begin{align}
c_{ss}^{(0)} &= g_N + \delta + \mathbb{I}_{s\in\{2,4,6\}}\Psi_hf_h + \frac{g_Q}{\log\lambda/m}, \\
c_{ss}^{(1)} &= - \frac{g_Q}{\log\lambda/m},
\end{align}$$ where $\mathbb{I}_{s\in\{2,4,6\}}=1$ if $s\in\{2,4,6\}$. All other elements $c_{ss}^{(j)}$ equal zero. The diagonals of $C_{12}$, $C_{36}$, and $C_{54}$ are $$\begin{equation}
c_{12}^{(0)} = c_{36}^{(0)} = c_{54}^{(0)} = -\Psi_hf_h.
\end{equation}$$ For $\{h,h\}$ we have non-zero elements of $C_{31}$, $C_{33}$, and $C_{35}$ equal to $$\begin{equation}
c_{31}^{(-m)} = c_{33}^{(-m)} = c_{35}^{(-m)} = -\Phi_{{\rm q}h}(\nu_h+f_h).
\end{equation}$$ Similarly, for $\{\ell,\ell\}$, $$\begin{equation}
c_{42}^{(-m)} = c_{44}^{(-m)} = c_{46}^{(-m)} = -\Phi_{{\rm q}\ell}(\nu_\ell+f_\ell).
\end{equation}$$ And finally, for $\{h,\ell\}$, $$\begin{equation}
c_{52}^{(-m)} = c_{54}^{(-m)} = c_{56}^{(-m)} = -\Phi_{{\rm q}h}(\nu_h+f_h),
\end{equation}$$ and $$\begin{equation}
c_{61}^{(-m)} = c_{63}^{(-m)} = c_{65}^{(-m)} = -\Phi_{{\rm q}\ell}(\nu_\ell+f_\ell).
\end{equation}$$ All other elements of $C$ are set equal to zero.[^85]
...
The convergence criterion is that the entire distribution $f_{i_1i_2\hat{q}}$ has converged between iterations. I implement this by checking whether $$\begin{equation}
||\mathbf{f}^k-\mathbf{f}^{k-1}||<\epsilon_{\rm outer}^{\rm tol},
\end{equation}$$ where $||\cdot||$ is the Euclidean norm and $\epsilon_{\rm outer}^{\rm tol}$ is a very small tolerance threshold (I choose $\epsilon_{\rm outer}^{\rm tol}=10^{-10}$). If this condition is satisfied, then the iteration is complete. Otherwise, use the distribution $\mathbf{f}^k$ to recompute the aggregates $\{\bar{a}^k,\mathcal{M}_{\eta-1}^k\}$ and begin iteration $k+1$.
...
Each firm has $n=\sum_{i_2\in\{h,\ell,\varnothing\}}n_{i_2}$ blueprints. Define the average quality of blueprints within the firm with followers $i_2$ as $$\begin{equation}
\tilde{q}_{i_2} \equiv \frac{1}{n_{i_2}}\sum_{j=1}^{n_{i_2}}e^{(\eta-1)\hat{q}_j}.
\end{equation}$$ We will show that $\{n_{i_2},\tilde{q}_{i_2}\}_{i_2\in\{h,\ell,\varnothing\}}\}$ is sufficient information for characterizing the full distribution of sales, profits, R&D and M&A expenditures, cash flows, and market values. The rescaled revenues of firm $i$ are $$\begin{equation}
...
\end{equation}$$ The firm-level markup is $$\begin{equation}
...
\end{equation}$$ and hence rescaled profits are $$\begin{equation}
\hat{\Pi}_{it} = \left(1-\frac{1}{\mu_{it}}\right)\frac{p_{it}y_{it}}{Y_t/L_t}.
\end{equation}$$ From this it is straightforward to see that cashflows equal $\hat{D}_{it} = \hat{\Pi}_{it} - \hat{w}_{\rm x}x_i$, and that rescaled firm value equals $$\begin{equation}
\hat{V}_{it} = \sum_{i_2\in\{h,\ell,\varnothing\}}n_{i_2}(v_{i_1i_2}^{\rm P}\tilde{q}_{i_2} + v_{i_1}^{\rm G}).
\end{equation}$$ These expressions confirm that the states $\{a_i,\{n_{i_2},\tilde{q}_{i_2}\}_{i_2\in\{h,\ell,\varnothing\}}\}$ are sufficient information for the variables in which we are interested.

The simulation exploits the fact that we know the distribution of ages and types $m_i(\tau)$ in closed-form. This means that, for each $a_i$, we can simulate a single cohort of firms from inception ($\tau=0$) to some terminal age ($\tau=\tau_{\rm max}$), keeping track of moments conditional on $\{a_i,\tau\}$, and then use $m_i(\tau)$ to compute unconditional moments after simulating. I discretize the age grid with equal time increments $\Delta t$: $$\begin{equation}
\tau \in \{0,\Delta t,\dots,\tau_{\rm max}-\Delta t,\tau_{\rm max}\},
\end{equation}$$ where the maximum age $\tau_{\rm max}$ is chosen such that the truncated mass of firms above $\tau_{\rm max}$, $1-\int_{\tau_{\rm max}}^\infty m(\tau)d\tau$, is close to zero. For convenience, I re-index age $\tau$ to time $t$ and let time start at $t=0$. The simulation begins with $M_{\rm sim}$ firms for each $a_i$. At each time step, I store the firms' characteristics $\{n_{i_2 t},\tilde{q}_{i_2 t}\}_{i_2\in\{h,\ell,\varnothing\}}$.
...
At each time step $t>0$, we simulate the evolution of blueprint counts and quality statistics. At time $t+\Delta t$, the simulated number of monopoly products for a firm with $n_{\varnothing t}$ current blueprints is $$\begin{equation}
...
\end{equation}$$ is the number of type-$\varnothing$ blueprints added and $$\begin{equation}
J_{\varnothing,t+\Delta t}^-\sim\mathrm{Poisson}((\delta+\mathbb{I}_{i_1=\ell}\Psi_hf_h)n_{\varnothing t}\Delta t),
\end{equation}$$ is the number of type-$\varnothing$ blueprints lost. Changes in $n_h$ and $n_\ell$ are simulated accordingly. The average quality of the portfolio of monopoly products then evolves approximately according to the discretized law of motion $$\begin{equation}
...
\label{eq_app:sim_monopqual}
\end{equation}$$ The first term on the right-hand side is the current average quality, after depreciation. The second term is the change in average quality from new blueprints: $$\begin{equation}
...
\end{equation}$$ where the new blueprints' qualities are drawn independently from the distribution $$\begin{equation}
...
As shown above, the set $\{a_i,\{n_{i_2},\tilde{q}_{i_2}\}_{i_2}\}$ is sufficient information to compute a firm's (rescaled) sales, profits, and value. Any moment (mean, covariance, etc.) of any function these variables at time step $t$ (age $\tau$) can be expressed in terms of expected values (means). For example, to compute the cross-sectional variance of firm markups, we can use the fact that $$\begin{equation}
\textrm{var}(\mu_{it}|a_i,\tau) = \mathbb{E}[\mu_{it}^2|a_i,\tau] - \mathbb{E}[\mu_{it}|a_i,\tau]^2,
\end{equation}$$ computing and storing each of these two expectations at each iteration of the simulation. Then, after the simulation, we can use the distribution $m_i(\tau)$ to compute $$\begin{equation}
...
\end{equation}$$ The same logic applies to any moment of the firm distribution in which we are interested.
...
The relevant state space for a transition path includes both the balanced-growth states $\{i_1,i_2,\hat{q}\}$ and the new time dimension $t\geq t_0$. Numerically, we can discretize this time grid into increments $\Delta t$ as $$\begin{equation}
t\in\{t_{\rm min},t_{\rm min}+\Delta t,\dots,t_{\rm max}-\Delta t,t_{\rm max}\},
\end{equation}$$ where $t_{\rm min}=t_0$ and $t_{\rm max}$ is sufficiently large that the economy has converged near the new steady state. The solution method then takes as its terminal condition ($t=t_{\rm max}$) the balanced-growth equilibrium, solved as described in Appendix 13.1 above.
...
\end{equation}$$ To solve the initial HJB equations, we need a sequence $\{\Phi_{{\rm n}i_1t},\Phi_{{\rm q}i_1t},\Psi_{i_1t},LN_t\}$ from $t=t_{\rm min}$ to $t=t_{\rm max}-\Delta t$. I therefore similarly interpolate $\{v_{i_1i_2\hat{q}t}^{\rm P}\}$ and $\{v_{i_1t}^{\rm G}\}$, then use these interpolated series to compute an initial sequence $\{\Phi_{{\rm n}i_1t}^0,\Phi_{{\rm q}i_1t}^0,\Psi_{i_1t}^0,LN_t^0\}$.

### Solving HJBs backwards

HJBs are solved backward, starting from the known terminal values $\{v_{i_1i_2\hat{q}t_{\rm max}}^{\rm P}\}$ and $\{v_{i_1t_{\rm max}}^{\rm G}\}$. The HJB equation for product values is $$\begin{equation}
...
\end{equation}$$ Approximating the time derivative with a forward difference implies, at iteration $k$, $$\begin{equation}
...
\end{equation}$$ Like in Appendix 13.1, the value of growth opportunities $v_t^{\rm G} = [v_{ht}^{\rm G}\quad v_{\ell t}^{\rm G}]^\top$ solves the two-by-two linear system $$\begin{equation}
...
\end{equation}$$ with entries $$\begin{align}
...
\end{equation}$$ Next, we solve forward for the total blueprint share $f_{h,t+\Delta t}^k$ using the aggregated KFE $$\begin{equation}
\frac{\partial}{\partial t}f_{ht} + g_{Nt}f_{ht} = (\Phi_{{\rm n}ht} + \Phi_{{\rm q}ht})(\nu_{ht} + f_{ht}) + \Psi_{ht}f_{ht}f_{\ell t} - \delta_tf_{ht}.
\end{equation}$$ Discretizing this expression over time with a forward difference implies that the distribution at time $t+\Delta t$ in iteration $k$ equals $$\begin{equation}
...
The convergence criterion is that the entire distribution $f_{i_1i_2\hat{q}t}$ has converged over the entire time path between iterations. I implement this by checking whether the average error at any given point in time $$\begin{equation}
\frac{||\mathbf{f}^k-\mathbf{f}^{k-1}||}{\mathrm{dim}(t)}<\epsilon_{\rm outer}^{\rm tol},
\end{equation}$$ where $||\cdot||$ is the Euclidean norm, $\mathrm{dim}(t) \equiv 1+(t_{\rm max}-t_{\rm min})/\Delta t$ is the number of points in the time grid, and $\epsilon_{\rm outer}^{\rm tol}$ is the same (very small) tolerance threshold used in the balanced-growth solution algorithm. If this condition is satisfied, then the iteration is complete. Otherwise, we reinitiate at iteration $k+1$.
...
In the second step, we simulate this panel forward in time $t\in\{t_0,\dots,t_{\rm end}-\Delta t,t_{\rm end}\}$. The method for simulating a firm forward and for drawing a cohort of new firms is, again, the same as in Appendix 13.2 above, the only difference being that at each point in time we now use the updated equilibrium path of the distribution, aggregates, and firm policies and values. To determine the number of new firms in each cohort, we need to compute the evolution of the entry rate ([\[eq_app:entrate_t\]](#eq_app:entrate_t)). At the beginning of the transition ($t=t_0$), we have a constant ratio $N_t/M_t$ and constant entry and exit rates $\mathcal{E}_{\rm entry}$ and $\mathcal{E}_{\rm exit}$ from the balanced-growth path. Over the transition, the exit rate $\mathcal{E}_{{\rm exit},t+\Delta t}$ equals the number of exiting firms divided by the total number of firms (per unit of time $\Delta t$). The number of blueprints per firm evolves as $$\begin{equation}
\frac{N_{t+\Delta t}}{M_{t+\Delta t}} = \frac{N_t}{M_t}\exp\{(g_{Nt}-(\mathcal{E}_{{\rm entry},t}-\mathcal{E}_{{\rm exit},t}))\Delta t\}.
\end{equation}$$ We can then use this ratio to compute the entry rate $\mathcal{E}_{{\rm entry},t+\Delta}$, which determines the number of new firms that must be simulated.

# Details of model estimation 

## Estimation procedure 

I estimate the parameters by simulated method of moments (SMM). In the first step, I search for the parameters that minimize the distance between the model-implied moments and data moments. Recall that $\Theta$ denotes the vector of estimated parameters. Let $\hat{m}$ denote the vector of empirical moments and $m(\Theta)$ the corresponding moments in the model. Let $\Sigma$ denote the sample covariance matrix of moments $\hat{m}$. Then the SMM estimator is defined as $$\begin{equation}
\hat{\Theta} = \mathop{\arg\!\min}\limits_{\Theta} \: \bigl\{(m(\Theta) - \hat{m})^\top \Sigma (m(\Theta) - \hat{m}) \bigr\}.
\end{equation}$$ I find this minimum-distance estimate using the numerical approach recently proposed by , which has three steps.[^86] First, I solve and simulate the model for 3000 pseudorandom combinations of parameters generated from a Halton sequence. Second, using the resulting dataset, I fit a cubic polynomial (with interactions) between parameters and moments. To increase precision around the correct estimate, I apply OLS weights proportional to the inverse squared error of each point. Third, using this fitted model, I apply the Nelder-Mead simplex algorithm to locally search for the minimum-distance estimate $\hat{\Theta}$.

I compute standard errors of moments using the following asymptotic results. Let $J$ denote the Jacobian matrix of moments with respect to parameters: for moment $m_j\in m(\Theta)$ and parameter $\theta_k\in\Theta$, element $(j,k)$ of the Jacobian equals $$\begin{equation}
J_{jk} = \frac{\partial m_j}{\partial \theta_k}.
\end{equation}$$ Standard asymptotic results imply that the (estimated) covariance matrix of parameters is given by $$\begin{equation}
\hat{\Omega} = (J^\top \Sigma^{-1} J)^{-1}.
\end{equation}$$ Standard errors are then given by the square root of the diagonal elements of $\hat{\Omega}$: $$\begin{equation}
\textrm{se}(\hat{\Theta}) = \sqrt{\textrm{diag}(\hat{\Omega})}.
\end{equation}$$ Numerically, I approximate the derivatives in $J$ using a double-sided finite difference with increments equal to 1% of each estimated parameter value (**judd1998numerical?**). I discuss the computation of the matrix $\Sigma$ next.

In the estimation of the balanced-growth path, $\Theta$ is the vector of parameters ([\[eq:estparams_bge_theta\]](#eq:estparams_bge_theta)) and $\hat{m}$ and $m(\Theta)$ the corresponding data moments. In the transition estimation, $\Theta$ is the vector of parameters $\{\kappa_\varphi,t_{\rm min},\varphi_\infty\}$ governing ([\[eq:varphi_transfunction\]](#eq:varphi_transfunction)) and the moments are the 45 values of aggregate R&D-to-value $\{w_{{x}t}X_{{\rm r}t}/V_t\}$ from 1975--2020.
...
Finally, for the sales-weighted standard deviations of value-to-sales and profit-to-sales, I rescale each variable by the corresponding aggregate value to remove level effects. For example, for value-to-sales, the moment (in the model notation) is equal to $$\begin{equation}
...
\end{align}$$ Substituting in the equilibrium wage and dividing by value then implies $$\begin{equation}
...
\end{equation}$$ which reduces to the equation in the main text if $\zeta=0$. Thus, if we again apply the intuition that $\Delta\log\bar{v}_{\rm r}\approx \Delta\log(V_t/N_t)$ and that the substitution term is insensitive to parameter changes, we have that $$\begin{align}
...
\end{equation}$$ Thus, we can infer the coefficient $\theta$ as $$\begin{equation}
\theta = 1 - \frac{\log(s_L(\bar{L})/s_L(\underline{L}))}{\log(\bar{L}/\underline{L})}.
\end{equation}$$ I compute this in the Business Dynamics Statistics, which reports the employment shares for employment bins in each year; I use $\underline{L}=5000$ and $\bar{L}=10000$. The empirical moment used in the estimation is then the average of annual estimates $\hat{\theta}_t$ from 1978--1985.
...
& & & & (0.0509) & & & & (0.0303) & &\
High-prod. entrep. share & & $\omega$ & & 0.0011 & & & 1.2786 & & 1.2793\
& & & & (0.0008) & & & & (0.0027) & &\

Table [\[tab:estimparams_alt_app\]](#tab:estimparams_alt_app) reports the results of this alternative estimation.[^88] Most of the parameters take very similar values to the baseline estimation. The exception is $\omega$, which takes a much lower value. The reason is that there is substantial concentration in the data. Indeed, as the Pareto tail approaches the lower bound $\theta=1$ (Zipf's law), which represents maximal concentration, the estimated value of $\omega$ approaches zero.[^89]
...
Consider the assumptions ([\[eq:prodfun_zshock\]](#eq:prodfun_zshock))--([\[eq:utility_duffieepstein\]](#eq:utility_duffieepstein)). Letting $Z_t=e^{z_t}$, we have $$\begin{equation}
\frac{dZ_t}{Z_{t^-}} = \left(\kappa_z(\bar{z} - z_t) + \frac{1}{2}\sigma_z^2\right)dt + \sigma_zdB_t + (e^{-\zeta_z}-1)dJ_t.
\end{equation}$$ For all values of $Z_t$, the static production allocations across firms will be the same as in the case without systematic risk, and aggregate output will equal $$\begin{equation}
Y_t = N_t^{1/(\eta-1)}Q_t\bar{a}_tZ_t\Omega_tL_{{\rm p}t}.
\end{equation}$$ Hence, if the distribution is stationary, then expected per-capita output growth equals $$\begin{equation}
...
\end{equation}$$ Expected growth is affine in $z$, so we should expect the same of asset prices.

Asset prices can be fully characterized by trading in a riskfree bond and the (per-capita) consumption claim. Let $V_{yt}$ denote the value of this claim. The return on the consumption claim is, by definition, its yield plus its capital gains: $$\begin{equation}
dR_t = \frac{y_t}{V_{yt}}dt + \frac{dV_{yt}}{V_{yt^-}} = (r_{ft} + \nu_t)dt + \sigma_{Vt}dB_t +  (e^{-\zeta_{Vt}}-1)dJ_t,
\end{equation}$$ for some risk premium $\nu$ and risk coefficient $\sigma_{V}$ and $\zeta_{V}$, for which we will solve. To simplify the proof, first conjecture that risk premia are constant ($\nu_t=\nu$) and the stock's risk coefficients are constants ($\sigma_{Vt}=\sigma_V$ and $\zeta_{Vt}=\zeta_V$), which follow from i.i.d. risks.

Letting $\alpha_t$ denote the share of the household's wealth $W_t$ invested in the consumption claim, the household's wealth evolves according to[^90] $$\begin{equation}
\frac{dW_t}{W_{t^-}} = \left(r_{ft}+\alpha_t\nu-\frac{c_t}{W_t}\right)dt + \alpha_t\sigma_VdB_t + \alpha_t(e^{-\zeta_V}-1).
\end{equation}$$ The household's HJB equation (suppressing time subscripts) is then $$\begin{multline}
...
\end{multline}$$ The first-order condition for consumption is $$\begin{equation}
U_W = u_c(c,l,U) = \rho(1-\gamma)\frac{U}{c},
\end{equation}$$ and that for the portfolio share is $$\begin{equation}
0 = U_WW\nu + U_{WW}W^2\alpha\sigma_V^2 + U_{Wz}W\sigma_V\sigma_z + p_zU_W(W(1+\alpha(e^{-\zeta_V}-1)),z-\zeta_z)W(e^{-\zeta_V}-1).
\end{equation}$$ Conjecture that the solution to this system is $$\begin{equation}
U(W,z) = \frac{W^{1-\gamma}}{1-\gamma}\exp\bigl\{(1-\gamma)(a_z+b_zz)\bigr\},
\end{equation}$$ for constants $a_z$ and $b_z$. Under this conjecture, we have $$\begin{equation}
c = \rho W
\end{equation}$$ and $$\begin{equation}
0 = \nu - \gamma\alpha\sigma_V^2 + (1-\gamma)b_z\sigma_V\sigma_z + p_z[(1+\alpha(e^{-\zeta_V}-1))^{-\gamma}(e^{-\zeta_V}-1)e^{-(1-\gamma)b_z\zeta_z}].
\end{equation}$$ Households consume a constant fraction of their wealth, the standard unit-EIS result. Their portfolios are governed by the classic risk-return tradeoff and a hedging demand against shocks to aggregate productivity $z$. Market clearing requires that the bond be in zero net supply, so we can set $\alpha=1$ and $\sigma_V=\sigma_z$ and $\zeta_V=\zeta_z$ and solve for the risk premium $$\begin{equation}
\nu = (\gamma+(\gamma-1)b_z)\sigma_z^2 + p_z[e^{(\gamma+(\gamma-1)b_z)\zeta_z}(1-e^{-\zeta_z})],
\end{equation}$$ which is indeed a constant, as conjectured. Standard asset-pricing results (see, e.g., ) and market clearing $c=y$ imply that the riskfree interest rate equals $$\begin{equation}
r_{ft} = \rho + g_{yt} - \gamma\sigma_z^2 - p_z[e^{\gamma\zeta_z}(1-e^{-\zeta_z})],
\end{equation}$$ where $g_{yt}$ is defined above. Interest rates are increasing in impatience $\rho$ and growth $g_{yt}$ and decreasing in precautionary savings demand from risk and risk aversion. Putting these together, the total expected return on the stock is $$\begin{equation}
r_{ft} + \nu = \rho + g_{yt} + (\gamma-1)b_z\sigma_z^2 + p_z[e^{\gamma\zeta_z}(e^{(\gamma-1)b_z\zeta_z}-1)(1-e^{-\zeta_z})].
\end{equation}$$ If $b_z\in(-1,0)$ (which I show below), then the risk premium is strictly increasing in risk aversion but the total expected return is decreasing in risk aversion, due to the dominance of the interest-rate decline.
...
\end{multline}$$ Substituting in $r_f$, $g_y$, and $\nu$, then collecting coefficients on $z$, we get $$\begin{equation}
b_z = -\frac{\kappa_z}{\kappa_z+\rho} \in (-1, 0).
\end{equation}$$ Collecting constants, we get $$\begin{multline}
...
\end{multline}$$ Substituting $b_z$ back into the risk premium expression implies $$\begin{equation}
...
\end{equation}$$ This verifies the conjectured value function and completes the household's problem.

To price firms, it is useful to first derive an expression for the law of motion of the state-price density $\xi$ of the representative household. Using the results of , we have $$\begin{equation}
\frac{d\xi_t}{\xi_t} = -r_{ft}dt - (\gamma+(\gamma-1)b_z)\sigma_zdB_t + (e^{(\gamma+(\gamma-1)b_z)\zeta_z}-1)(dJ_t-p_zdt).
\end{equation}$$ By the absence of arbitrage, it must be that $$\begin{equation}
r_{ft}dt = -\mathbb{E}_t\frac{d\xi_t}{\xi_t},
\end{equation}$$ and that $$\begin{equation}
\nu dt = -\mathbb{E}_t\frac{d[\xi,y]_t}{\xi_ty_t},
\end{equation}$$ both of which are consistent with the results above.

Finally, let us assess the consequences of risk and risk aversion for firms' valuations and decisions. Recall that the value of the firm equals $$\begin{equation}
V_{it} = \mathbb{E}_t\left[\int_0^\infty\frac{\xi_{t+\tau}}{\xi_t}D_{i,t+\tau}d\tau\right],
\end{equation}$$ which we can scale by $y_t$ to get $$\begin{equation}
\hat{V}_{it} = \mathbb{E}_t\left[\int_0^\infty\frac{\xi_{t+\tau}}{\xi_t}\frac{y_{t+\tau}}{y_t}\hat{D}_{i,t+\tau}d\tau\right],
\end{equation}$$ where the scaled dividend equals $$\begin{equation}
\hat{D}_{it} = \sum_{j=1}^n\pi_{ijt} - \hat{w}_{{\rm x}t}x_{it}n_{it},
\end{equation}$$ and is thus independent of $y$ and $Z$. After some algebra, this expression can be rewritten $$\begin{equation}
...
\end{equation}$$ and so the expression on the left-hand side is a martingale. It is hence a local martingale, and has zero expected drift: $$\begin{equation}
0 = \xi_ty_t\hat{D}_{it}dt + \mathbb{E}_t[d(\xi_ty_t\hat{V}_{it})].
\end{equation}$$ Because $\hat{V}$ is independent of $\xi$ and $y$, this expression can be rewritten $$\begin{equation}
0 = \hat{D}_{it}dt + \mathbb{E}_t\left[\frac{d(\xi_ty_t)}{\xi_ty_t}\right]\hat{V}_{it} + \mathbb{E}_t[d\hat{V}_{it}].
\end{equation}$$ Using the household's state-price density along with no-arbitrage conditions, we know $$\begin{align}
...
&= -(r_{ft} + \nu - g_{yt})dt \\
&\equiv - \rho^*dt,
\end{align}$$ which is a constant independent of the state $z_t$ (and of the expected growth rate $g_{yt}$). Substituting this back in, we have the scaled, risk-adjusted HJB equation $$\begin{equation}
\rho^*\hat{V}_{it} = \hat{D}_{it} + \mathbb{E}_t[d\hat{V}_{it}],
\end{equation}$$ where the firm's expected growth rate is of the exact same form as it was with log utility and no aggregate risk. This proves that $\hat{V}$ is independent of aggregate state variables.

The only difference between this model and the riskless model is that, where there was once discounting by the constant $\rho$, there is is now discounting by the risk-adjusted constant $$\begin{equation}
...
\end{equation}$$ The first-order condition for the capital-labor ratio is $$\begin{equation}
\beta w_{{\rm p}t} k_{ijt}^{-1-\beta} = (1-\beta)r_{Kt}k_{ijt}^{-\beta},
\end{equation}$$ and therefore equals $$\begin{equation}
k_{ijt} = \frac{\beta}{1-\beta}\frac{w_{{\rm p}t}}{r_{Kt}}.
\end{equation}$$ This also means that $$\begin{equation}
...
\end{equation}$$ so the quality-adjusted marginal cost of a firm is $$\begin{equation}
mc_{ijt} = \frac{1}{q_{ijt}a_i}\left(\frac{w_{{\rm p}t}}{1-\beta}\right)^{1-\beta}\left(\frac{r_{Kt}}{\beta}\right)^\beta.
\end{equation}$$ Hence, the optimal markup for the leader $i_1$ is the same as in the case without capital: $$\begin{equation}
\mu_{i_1jt} = \min\left\{\frac{a_{i_1}q_{i_1jt}}{a_{i_2}q_{i_2jt}},\frac{\eta}{\eta-1}\right\}.
\end{equation}$$ All of this implies that the labor and capital shares of revenue for product $j$ are $$\begin{equation}
...
\end{equation}$$ respectively. Consequently, the aggregate expenditure shares of output will equal $$\begin{equation}
\Lambda_{Lt} = \frac{w_{{\rm p}t}L_{{\rm p}t}}{Y_t} = (1-\beta)\Lambda_t \quad\textrm{and}\quad \Lambda_{Kt} = \frac{r_{Kt}K_t}{Y_t} = \beta\Lambda_t
\end{equation}$$ with $\Lambda_t$ equal to an inverse of the aggregate markup, as in the labor-only benchmark model.

Capital is accumulated according to $$\begin{equation}
\dot{K}_t = I_t-\delta K_t.
\end{equation}$$ The objective of the capital sector is to choose a capital investment policy $I_t$ to maximize the present value $V_{Kt}$ of its rental income less investment cost, $r_{Kt}K_t-I_t$, where the rental rate $r_{Kt}$ is taken as given since the capital sector is perfectly competitive. The total value of the sector thus satisfies the HJB equation $$\begin{equation}
r_{ft}V_{Kt} = r_{Kt}K_t - I_t + \frac{\partial V_{Kt}}{\partial K_t}(I_t-\delta K_t).
\end{equation}$$ The investment first-order condition is $$\begin{equation}
\frac{\partial V_{Kt}}{\partial K_t} = 1,
\end{equation}$$ which implies $$\begin{equation}
V_{Kt} = \frac{r_{Kt} - \delta}{r_{ft}}K_t = K_t,
\end{equation}$$ and hence that $$\begin{equation}
r_{Kt} = r_{ft}+\delta.
\end{equation}$$ Using the fact that the aggregate capital expenditure share equals, $$\begin{equation}
\Lambda_{Kt} = \frac{r_KK_t}{Y_t} = \beta\Lambda,
\end{equation}$$ it must be that the growth rate of capital equals $$\begin{equation}
g_{Kt} = g_{Yt} + g_{\Lambda t},
\end{equation}$$ where $g_{\Lambda t}=\dot{\Lambda}_t/\Lambda_t$ is zero on a balanced-growth path.[^91] Thus, the equilibrium investment-capital ratio equals $$\begin{equation}
\frac{I_t}{K_t} = \delta + g_{Yt} + g_{\Lambda t}.
\end{equation}$$ Higher total output growth, either through technological innovation or population growth, necessitates more capital, increasing the intensity of capital investment. If $\Lambda_t$ rise (markups fall), then firms increase investment to increase the capital share. Along the transition path, the investment rate will therefore fall both because the rate of economic growth falls and because the aggregate markup rises.

The only remaining change in the model is the fact that resources must now be allocated to the creation of new capital. Hence, total consumption will equal $$\begin{equation}
C_t = Y_t - I_t  = \left(1 - \frac{I_t}{Y_t}\right) Y_t.
\end{equation}$$ Because $I_t/Y_t$ is a constant in a stationary equilibrium, consumption growth and output growth are identical, preserving this characteristic of the baseline model.

This decline in investment intensity amid booming market valuations aligns with the literature studying the divergence of average $Q$ (the value of the stock market relative to capital) and marginal $q$ of investment (**crouzet2023rents?**). In this model, average $Q$ rises, for the reasons laid out in the paper, while marginal $q$ always equals $\partial V_K/\partial K=1$. The reason is that, in this model, most of the value of the market comes from intangible capital---blueprints, generated by R&D investment---and it is the present value of profits from intangible capital that drives average $Q$ up. Physical capital merely serves to scale up production once new ideas are in place, and therefore stagnates as output growth stagnates.

## Cournot competition 

Suppose firms choose quantities $Y_{ijt}$ instead of prices $p_{ijt}$ when they operate within a product market. The conditions for household demand remain the same, so $$\begin{equation}
p_{ijt} = q_{ijt}\left(\frac{Y_{jt}}{Y_t}\right)^{-1/\eta} = q_{ijt}p_{jt}.
\end{equation}$$ The profit maximization problem of a firm becomes $$\begin{align}
...
\end{align}$$ The first-order condition for an interior solution is $$\begin{equation}
p_{jt} - \frac{w_{{\rm p}t}}{a_iq_{ijt}} = \frac{1}{\eta}p_{jt}^{\eta+1}\frac{q_{ijt}Y_{ijt}}{Y_t}.
\end{equation}$$ Summing over the $m_{jt}$ producing firms $i$ then implies $$\begin{equation}
...
\end{equation}$$ is a harmonic average of quality-adjusted productivity in the product market. Substituting in the household demand curve then implies that the product-level price equals $$\begin{equation}
p_{jt} = \underbrace{\frac{\eta m_{jt}}{\eta m_{jt}-1}}_{\equiv\mu_{jt}} \frac{w_{{\rm p}t}}{\overline{aq}_{jt}},
\end{equation}$$ where we have defined $\mu_{jt}$ as the markup over product-level marginal cost. Because all producers must set $p_{ijt}=q_{ijt}p_{jt}$, firm $i$'s markup over marginal cost equals $$\begin{equation}
...
\end{equation}$$ Among the $m_{jt}$ producing firms, those with higher quality-adjusted productivity relative to the average will earn higher markups.

The question that remains is which firms will choose to produce and which will decide not to operate. That is, what determines $m_{jt}$? A firm will choose to produce ($Y_{ijt}>0$) if and only if it will earn positive profits, or, equivalently, a markup greater than one: $$\begin{equation}
\frac{\eta m_{jt}}{\eta m_{jt}-1}\frac{a_iq_{ijt}}{\overline{aq}_{jt}} \geq 1.
\label{eq_all:cournot_prodcondition}
\end{equation}$$ Under our parameterization $\lambda>a_h/a_\ell$, the markups of producers are exactly ranked by their quality rankings. Consequently, the equilibrium number of producers $m$ is the number such that (i) the $m$th highest-quality firm earns a positive markup and (ii) the $(m+1)$th highest-quality firm would have a negative markup if it decided to produce.

This intuition can be used to find all possible equilibrium market structures. First, note that, given any set of potential producers, a unique equilibrium exists with a finite number of producing firms. Formally, there exists a finite $\bar{m}\in\mathbb{R}$ such that, for any combination of high- and low-productivity blueprint holders, no product market will ever have $m_{jt}>\bar{m}$ producing firms. This maximum occurs when the lowest-quality firm is high-productivity and all higher-quality blueprint holders are low-productivity (since, given $m$, this maximizes $a_iq_{ijt}/\overline{aq}_{jt}$ for the worst firm). In this case, the necessary condition for participation is $$\begin{equation}
\frac{\eta\bar{m}}{\eta\bar{m}-1}\frac{1}{\bar{m}}\left(1 + \frac{a_\ell}{a_h}\sum_{k=1}^{\bar{m}-1}\lambda^{-k}\right) \geq 1,
\end{equation}$$ or, evaluating this sum, $$\begin{equation}
\frac{a_\ell}{a_h}\frac{\lambda^{-1}-\lambda^{-\bar{m}}}{1-\lambda^{-1}} \geq \bar{m} - \frac{\eta+1}{\eta}.
\end{equation}$$ The maximum number of possible producers is then the value of $\bar{m}$ that makes this an equality.[^92] Second, note that, under the parameter estimates in the main model, we will have $\bar{m}>2$. To see this, substitute $m=2$ into ([\[eq_all:cournot_prodcondition\]](#eq_all:cournot_prodcondition)), to see that the condition for the follower ($a_i=a_{i_2}$) to produce is $$\begin{equation}
\frac{\eta}{\eta-1} > \lambda\frac{a_{i_1}}{a_{i_2}},
\end{equation}$$ which always holds in the estimated Bertrand model.
...
\label{eq_app:hjb_productvalue_switching}
...
\label{eq_app:hjb_growthvalue_switching}
\end{multline}$$ The second and only other alteration we must make is to the KFEs ([\[eq_app:kfe_monop\]](#eq_app:kfe_monop)), ([\[eq_app:kfe_sametype\]](#eq_app:kfe_sametype)), and ([\[eq_app:kfe_opptype\]](#eq_app:kfe_opptype)), which become $$\begin{multline}
\dot{f}_{i\varnothing\hat{q}t} + g_{Nt}f_{i\varnothing\hat{q}t} = \frac{\partial f_{i\varnothing\hat{q}t}}{\partial \hat{q}}g_{Qt} + \Phi_{{\rm n}it}(f_{it} + \nu_{it})\Gamma_{\hat{q}} \\
...
\label{eq_app:kfe_monop_switching}
\end{multline}$$ $$\begin{multline}
\dot{f}_{ii\hat{q}t} + g_{Nt}f_{ii\hat{q}t} = \frac{\partial f_{ii\hat{q}t}}{\partial \hat{q}}g_{Qt} + \Phi_{{\rm q}it}(f_{it} + \nu_{it})(f_{ii,\hat{q}-\log\lambda,t}+f_{i,-i,\hat{q}-\log\lambda,t}+f_{i\varnothing,\hat{q}-\log\lambda,t}) \\
...
\label{eq_app:kfe_sametype_switching}
\end{multline}$$ $$\begin{multline}
\dot{f}_{i,-i,\hat{q}t} + g_{Nt}f_{i,-i,\hat{q}t} = \frac{\partial f_{i,-i,\hat{q}t}}{\partial \hat{q}}g_{Qt} + \Phi_{{\rm q}it}(f_{it} + \nu_{it})(f_{-ii,\hat{q}-\log\lambda,t}+f_{-i,-i,\hat{q}-\log\lambda,t}+f_{-i\varnothing,\hat{q}-\log\lambda,t}) \\
...
[^3]: I find a decline of similar magnitude in the aggregate market value of patents (**kogan2017technological?**), meaning that this fact is true of both research outputs and inputs.
...
[^6]: Beyond valuations, shows that this is essential for matching the firm-size distribution. It also implies a long-run link between population and economic growth (**jones1995models?**; **jones2022population?**).
...
[^10]: and focus on the puzzle that interest rates have plummeted since 1980 but the market has not risen commensurately. argue that the equity premium rose while argue that the true riskfree rate did not fall as much as real yields on government debt. Both of these stories are qualitatively consistent with my findings. Like , I argue that a decline in growth can help reconcile the performance of stocks and bonds.

[^11]: This is consistent with the argument of that intangibles (in this case, incumbents' stock of ideas) drove a wedge between "average $Q$\" and "marginal $q$.\"
...
[^14]: My estimate of a roughly $50\%$ total decline in research productivity since 1975 is smaller in magnitude than, but still reasonably close to, most of the micro and macro estimates in .

[^15]: assume the stock market is a levered consumption claim, thus making the critical distinction between the profit claim and human capital. They do not, however, distinguish between incumbents' profits and future entrants' profits, which I find to be essential.

[^16]: They find that declining population growth initially boosts growth before lowering it. Feeding the population-growth decline into my model would imply both a short- and a long-run growth decline. This could also generate the more extreme prediction of of the "end of economic growth.\"

[^17]: In , growth falls because of decreasing returns to R&D within firms: rising concentration reduces the productivity of the average researcher, rendering new ideas effectively harder to find.

[^18]: also seek to explain the empirical fact that markups have fallen within firms, yet aggregate markups have risen because of a reallocation to high-markup firms (**kehrig2021micro?**; **deloecker2020rise?**; **autor2020fall?**). My model also generates diverging micro and macro trends, because the high-productivity firms that gain market share are also high-markup firms, but have to compete more with each other as they gain market share.
...
[^23]: I also show how the decomposition ([\[eq:valsale_decomp\]](#eq:valsale_decomp)) can be constructed from the decomposition that is commonly used in macroeconomics, and how the adjustments I make are important for addressing econometric biases that emerge in this class of decomposition when studying secular change in valuations.

[^24]: This is consistent with the findings of and that the U.S. listing gap is in part explained by the increasing M&A of public targets by public acquirers. In other words, the "missing firms\" in public markets are still operating under the ownership of public acquirers.

[^25]: Note that this assumption is not the same as a decentralized economy with perfect consumption insurance (complete markets), because entrepreneurial decisions would be distorted by a moral hazard problem from redistributing private gains (**Jones2022taxing?**). One could consider an economy without consumption insurance, but then the distribution of households would become a state variable. The assumption of a family, in contrast, yields essentially complete markets without distorting private incentives for firm creation.

[^26]: It is common in endogenous-growth models to put the good-specific product quality $q_{ijt}$ into the production function ([\[eq:model_prodfun\]](#eq:model_prodfun)) instead of the demand function ([\[eq:demand_withinvariety\]](#eq:demand_withinvariety)). These are isomorphic assumptions.
...
[^28]: An alternative assumption would be to have a single R&D technology with undirected innovations ex post: researchers come up with an idea, and then that idea happens to be a quality innovation with probability $\alpha$ (**peters2022population?**). Properly calibrated, this alternative would yield similar results.
...
[^31]: and [Fons-Rosen et al.]{style="color: blue"} (**fonsrosen2022effects?**) also model M&A as arising from endogenous search.
...
[^37]: A natural definition of the aggregate markup is thus $\Lambda^{-1}$, as in .
...
[^42]: For models that assume an adjustment cost instead of an idea-production function, $\varepsilon=0.5$ corresponds to a quadratic cost function, which is also common (**klette2004innovating?**; **peters2020heterogeneous?**).
...
[^61]: $\kappa_z>0$ implies that disasters are followed by recoveries (**nakamura2013crises?**).
...
[^68]: These empirical results should be interpreted with caution for two major reasons. First, the gains from M&A must be measured relative to the counterfactual in which the transaction did not take place. Second, as point out, most of the large-bidder acquisitions with negative returns that drive the average results are concentrated in a few firms in 1999 and 2000, which may indicate a temporary period of overvaluation and also confounds acquirer-size and time fixed effects in empirical studies.
...
[^70]: This is not a problem for , who show that the cross term is close to zero for their markup series.

[^71]: propose a similar accounting for TFP. argue that the decomposition is more ideal for TFP accounting, because the cross term is meaningful in that context.
...
[^87]: An alternative approach could be to look at "worst-case\" entries for the unknown elements of the covariance matrix, as recently proposed by . This would have a minor effect on the main results, which turn out to be very robust to moments.
